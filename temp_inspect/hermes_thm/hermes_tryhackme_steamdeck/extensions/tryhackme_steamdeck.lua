...
-- Standalone CLI mode. If required as a module/Hermes extension, just return Extension.
if type(arg) == "table" and arg[0] and tostring(arg[0]):match("tryhackme_steamdeck%.lua") then
  ...
  elseif command == "vpn" then
    print_result(Extension.vpn(nil, arg[2] or "status"))
  elseif command == "uninstall" then
    print_result(Extension.uninstall(nil, { keep_profile = arg[2] ~= "--purge" }))
  else
    usage()
  end
else
