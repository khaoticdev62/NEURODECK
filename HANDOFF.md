...
**Epics 0–2 complete; Epics 3 and 4 partially complete by design** — each has explicitly deferred items behind documented backend dependencies, not silently skipped (see `docs/implementation/NDX_IMPLEMENTATION_LEDGER.md`'s "Scope decision" entries for both epics). The app has a real Gamepad API + keyboard input layer, a working Spatial Focus Engine, haptics, and a real AI-safety pipeline (typed tool registry, permission broker, audit log, action queue) demonstrated end to end with one genuinely real tool.
...
│   └── implementation/NDX_IMPLEMENTATION_LEDGER.md  (Epic 0/1/2/3/4 findings)
├── src/
...
│   │   ├── app/
│   │   │   ├── providers/AppProviders.tsx   (ErrorBoundary > Toast > FocusEngine > AiSafety > DisplayMode > Router)
│   │   │   ├── error-boundaries/RootErrorBoundary.tsx
│   │   │   ├── routing/ (routes.tsx, RouterRoot.tsx, EpicBoundaryPlaceholder.tsx)
│   │   │   └── shell/ShellLayout.tsx (+ __tests__) — mounts CoreToolsBootstrap, CommandPalette, ActivityAndNotificationsOverlay, EmergencyStopOverlay globally
│   │   ├── components/
...
│   │   ├── services/        (still empty ownership dir)
│   │   └── __tests__/App.test.tsx, testUtils.tsx (shared renderWithProviders helper, now wraps AiSafetyProvider too)
│   ├── shared/                     (contracts/schemas/errors/constants/types — empty)
│   └── core/                       (actions/permissions/models/... — empty; the AI safety pipeline lives in src/renderer/src/ai-safety/ instead, since it's renderer-only for now — see ledger)
...
npm run test         # 129/129 unit tests passing (was 93 at end of Epic 3)
npm run build        # electron-vite build succeeds (renderer: 26.13 kB CSS, 728.72 kB JS)
npm run test:e2e     # 1/1 Playwright Electron smoke test passing
```

## What to do next

1. **Epic 5 — Workspaces and files** is next in Phase A order. It also unblocks several deferred items: ND-006 Workspace Discovery (Epic 3), and gives the AI safety pipeline its first cross-process tool candidate (real file operations), which is when building the typed IPC layer (§14, deferred in Epic 4) finally has a load-bearing consumer.
2. Alternatively, **finish more of Epic 3/4's deferred screens** if a dependency becomes available sooner (e.g. if Epic 9's Model Router lands out of order, ND-013 AI Command Canvas and ND-005 AI Provider Setup both become buildable).
...
7. **`HapticsService` and `GamepadAdapter` both guard against `navigator.getGamepads` not existing** — apply the same `typeof navigator.getGamepads === 'function'` check to any new Gamepad API consumer.
8. **When adding a tool that needs main-process access** (filesystem, shell, network from Node) — that's the trigger to finally build §14's typed, Zod-validated IPC contract layer. Don't add ad hoc IPC channels around it; route through the same `ActionQueue`/`ToolRegistry` pipeline, with the tool's `run()` calling into the new IPC layer instead of a renderer-only service.
9. **Steam Input / native adapter remains an open gap** — physical Steam Deck rear grip buttons (L4/L5/R4/R5), Quick Access, and the Steam button aren't reachable via the standard Gamepad API. Revisit if/when that becomes a priority.
...
- **Typed cross-process IPC (§14) is still deferred** — the preload bridge still exposes `@electron-toolkit/preload`'s generic wrapper, unused by any feature. Build the real typed/Zod-validated layer the moment a tool needs main-process access (Epic 5/6), not before.
...
- **Epic 3's 6 deferred screens** each have a named blocking dependency (Epic 5 Workspace Service, Epic 8 task runtime, Epic 9 Model Router, Epic 10 profiles/credentials) — don't build them with fabricated data.
...
- Epic 0/1/2/3/4 evidence and decisions: `docs/implementation/NDX_IMPLEMENTATION_LEDGER.md`.
