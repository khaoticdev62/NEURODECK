...
│   (renderer) features/system/ — SystemDashboard.tsx (ND-042), PowerMenu.tsx (ND-051), AboutDiagnostics.tsx (ND-056) + __tests__
...
npm run test         # 339/339 unit tests passing across 72 files
...
4. **Recovery Service, System Metrics Service, ND-042 System Dashboard, ND-051 Power Menu, and ND-056 About/Diagnostics (Epic 11 slices) landed.** Recovery closed Epic 7's read-only gap — Build Studio now has real editing and save. System Metrics is real and consumed by both Epic 9's routing and the System Dashboard. Power Menu only wires the two safe app-lifecycle actions (restart/quit) — **do not add real OS-level suspend/reboot/shutdown without an explicit native-integration design and the user's sign-off first**, since those are irreversible against the whole host machine. The other 8 of 16 Epic 11 items (Controller Settings, Display/Theme, Network/VPN, Privacy, Integrations, Updates, Quick Access full build, Error Recovery) remain deferred — each needs its own service this slice didn't build.
...
- **The Recovery Service, System Metrics Service, ND-042 System Dashboard, ND-051 Power Menu, and ND-056 About/Diagnostics slices of Epic 11 are real; the other 8 of 16 Epic 11 items are not** — don't assume Controller Settings, Display/Theme, Network/VPN, Privacy, Integrations, Updates, Quick Access (full build), or Error Recovery exist just because "Epic 11" shows up as partially complete.
