...
> **Epic 9 completion update (2026-06-22):** The provider-connectivity-only limitation described later in this file is superseded. `ModelRouter` now performs real chat completion invocation, all eight routing profiles use live `SystemMetricsService` measurements, provider enable/disable is enforced, and configured Ollama endpoints expose capability-detected load/unload/status/benchmark controls. See `docs/implementation/NDX_EPIC_9_MODELS.md`. Epic 8 Agent Runtime remains a separate epic and is now unblocked.

**Epics 0–2 complete; Epics 3, 4, 5, 6, 7, and 9 partially complete by design; the Recovery Service slice of Epic 11 is real; Epic 8's Workflow Engine is real (Agent Runtime fully deferred).** `RecoveryService` (`core/recovery/`) records a real content-snapshot checkpoint before every file overwrite; `FileService.write()` is unconditionally checkpointed. This closed Epic 7's read-only gap: Build Studio (`/build`) has real editing and save. `/recovery` and `/storage` are real Recovery Service screens. `/automations` (Workflow Library), `/automations/forge` (Workflow Forge — a controller-friendly ordered step list, not a graph canvas), and `/automations/runs/:runId` (Workflow Run Detail) are real: `tool-action` steps run through the exact same Epic 4 `ActionQueue` pipeline as a Command Palette action, `user-approval` steps genuinely pause a run pending a real human decision, `condition`/`validator` use structured comparisons (never `eval()`). Agent Runtime (§17) is deferred in full — an agent with no model to plan with would be an empty shell, confirmed with the user before starting Epic 8. `/models` (Model Control Center) and `/models/:providerId` (Model Detail) are real, scoped to provider connectivity: a genuine OpenAI-compatible HTTP client with a real connection test and capability discovery against local (e.g. Ollama) or cloud endpoints, with API keys encrypted via Electron's `safeStorage` and never exposed to the renderer — there is no local model inference runtime, and Routing Profiles are deferred pending a System Metrics Service that doesn't exist yet. Local Git plus remote operations, ND-028 Direct terminal mode, and ND-029 Command Builder remain real partial slices from Epic 6. AI generation, real LSP for non-TS/JS languages, structural code editing, Git restore/discard/force-push, other file operations (copy/move/rename/delete), AI decision/Script/Parallel/Merge/Rollback workflow node types, and the other 15 of 16 Epic 11 screens remain incomplete.
...
│       └── models/ModelProviderStore.ts, ModelProviderService.ts, SecretCipher.ts (+ __tests__ — real encryption round-trip via injected cipher, real HTTP server connection tests)
...
│   (renderer) features/models/ — ModelControlCenter (ND-035), ModelDetail (ND-036) + __tests__
...
│       (agents/audit/browser/permissions/remote/system — still empty; the AI safety pipeline lives in src/renderer/src/ai-safety/ for now, see ledger)
...
npm run test         # 306/306 unit tests passing across 63 files
...
1. **Epic 9's Model Router landed, scoped to provider connectivity; the local model runtime and Routing Profiles remain deferred.** `ModelProviderService` is a real OpenAI-compatible HTTP client (real connection test, real capability discovery) and `ModelProviderStore` encrypts API keys via Electron's `safeStorage` before they ever touch disk. ND-037 Routing Profiles need real measured resource data (battery/thermal/memory) from a System Metrics Service that doesn't exist yet (Epic 11 §27) — don't build it against fabricated thresholds. ND-013 AI Command Canvas and ND-005 AI Provider Setup are now one step closer (provider connectivity exists) but still need an actual model/planner before they're buildable for real.
2. **Epic 8's Workflow Engine landed; Agent Runtime is fully deferred to Epic 9, and Epic 9 only delivered provider connectivity — Agent Runtime is still blocked.** An agent with no model to plan with would be an empty shell — confirmed this split with the user before starting Epic 8. Building Agent Runtime (§17, ND-016, ND-017) and the workflow engine's AI decision node type needs an actual model invocation path (send a prompt, get a completion), which this slice's `/models` endpoint connectivity doesn't provide on its own — that's the next real increment.
...
4. **Recovery Service (Epic 11 slice) landed, and it closed Epic 7's read-only gap.** Build Studio now has real editing and save. The other 15 of 16 Epic 11 items (System Metrics, Dashboard, Controller Settings, Display/Theme, Network/VPN, Privacy, Integrations, Updates, Quick Access full build, Power Menu, Error Recovery, About/Diagnostics) remain deferred — each needs its own service this slice didn't build.
