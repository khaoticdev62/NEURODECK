...
│       └── agents/AgentStore.ts, AgentRuntime.ts (+ __tests__ — real persisted definitions/runs, real planning via ModelRouter, real timeout/cancellation; no tool execution or IPC/UI yet)
...
│       (agents UI/audit/browser/permissions/remote — still empty; Agent Runtime core exists in core/agents/ with no UI yet; the AI safety pipeline lives in src/renderer/src/ai-safety/ for now, see ledger)
...
npm run test         # 312/312 unit tests passing across 66 files
...
2. **Epic 8's Workflow Engine is real, and Agent Runtime's core lifecycle (`core/agents/`) is now real too** (unblocked once Epic 9 landed a real model router). `AgentRuntime.start()` plans through `ModelRouter.complete()` with a real per-run timeout/cancellation and persists real token usage — but it only plans; there is no ActionQueue-backed tool execution, no typed IPC/preload/renderer wiring, and no ND-016/ND-017 screens yet. The workflow engine's AI decision node type is also now unblocked by the same model router but not yet wired up. Do not mark Agent Runtime or Epic 8 complete until tool execution (reusing the exact same registry → permission → approval → audit pipeline as the Workflow Engine, never a separate weaker path) and the UI land and are tested end-to-end.
...
- **The Recovery Service and System Metrics Service slices of Epic 11 are real; the other 14 of 16 Epic 11 items are not** — don't assume ND-042 System Dashboard, Controller Settings, Display/Theme, Network/VPN, Privacy, Integrations, Updates, Quick Access (full build), Power Menu, Error Recovery, or About/Diagnostics exist just because "Epic 11" shows up as partially complete. The metrics *core* exists; nothing exposes it to the UI yet (no shared contracts/IPC/preload for it).
- **Epic 8's Workflow Engine is real; Agent Runtime's core lifecycle is now real too, but it only plans — it does not execute tools, and has no UI.** ND-016 Agent Operations Center and ND-017 Agent Detail don't exist; neither does typed IPC for any of `core/agents/`. Don't claim Agent Runtime or Epic 8 complete until tool execution and the UI land.
