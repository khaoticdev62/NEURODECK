...
interface Finding {
  ...
  pattern: string;
  context: string;
  classification: 'production_violation' | 'safe_static_config' | 'allowed_fallback';
}
...
const ALLOWED_EXCEPTIONS: Array<{ file: string | RegExp; pattern: string; reason: string }> = [
...
function isAllowed(filePath: string, line: string): boolean {
  ...
  for (const a of ALLOWED_EXCEPTIONS) {
...
// Write JSON report
const outDir = path.join(ROOT, 'reports', 'cleanup');
fs.mkdirSync(outDir, { recursive: true });
fs.writeFileSync(
  path.join(outDir, 'mock-data-findings.json'),
  JSON.stringify({ scannedAt: new Date().toISOString(), violations, warnings, allowed: ALLOWED_EXCEPTIONS }, null, 2),
  'utf8'
);
...
console.log(`\nAllowed exceptions: ${ALLOWED_EXCEPTIONS.length}`);
console.log(`Report: reports/cleanup/mock-data-findings.json\n`);
