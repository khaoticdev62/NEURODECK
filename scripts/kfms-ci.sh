...
#
# Usage:
#   ./scripts/kfms-ci.sh run              — run all gates (sequential, safe)
#   ./scripts/kfms-ci.sh run --parallel   — run independent gates in parallel
#   ./scripts/kfms-ci.sh run --background — daemonize, write to .loose/inbox/kfms-ci/
#   ./scripts/kfms-ci.sh lint             — lint/format gates only
#   ./scripts/kfms-ci.sh test             — test gates only
#   ./scripts/kfms-ci.sh security         — security gates only
#   ./scripts/kfms-ci.sh build            — build gates only
#   ./scripts/kfms-ci.sh watch            — continuous mode on file changes
# =============================================================================
...
gate_kfms_meta() {
    cd "$ROOT"
    bash "$ROOT/scripts/khaotic-init.sh" validate
}
...
cmd_status() {
    ...
    if [[ -f "$REPORT_JSON" ]]; then
        ...
        warn "No report found. Run: ./scripts/kfms-ci.sh run"
    fi
}
...
case "$CMD" in
    ...
    *)
        ...
        echo ""
        echo "  Usage: ./scripts/kfms-ci.sh <command> [flags]"
        echo ""
        echo "  Commands:"
