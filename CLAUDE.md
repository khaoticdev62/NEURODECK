...
**Current repository state:** Epics 0–2 complete; Epics 3, 4, and 5 partially complete by design, and Epic 6 is active. Real Gamepad API + keyboard adapters, the Spatial Focus Engine, haptics, and a dev-only focus debug overlay exist and are tested. Real screens now include a partial ND-025 Git Control Center at `/git`: working tree, staged changes, exact unified diffs, editable/reviewed local commits, branches, and history are backed by real main-process Git operations. The same Git workstation is embedded in Workspace Detail. Remote Git, pull requests, recovery branches, AI commit assistance, recovery-dependent operations, and Terminal/PTTY remain incomplete. Steam Input/native-adapter support is still an explicit, documented gap. Routes without a real screen yet render an honest placeholder naming the screen ID and owning epic. See `HANDOFF.md` and `docs/implementation/NDX_IMPLEMENTATION_LEDGER.md` for current state and next steps.
