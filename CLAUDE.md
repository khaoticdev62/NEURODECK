...
**Current repository state:** Epics 0–2 complete; Epics 3, 4, and 5 partially complete by design, and Epic 6 is active. The partial ND-025 Git Control Center is real. ND-028 Universal Terminal now has a real Direct mode at `/terminal`: xterm.js renders workspace-scoped `node-pty` sessions with multiple-session switching, input, resize, ordered streaming/snapshot hydration, exit state, workspace/Git-branch context, controller focus nodes, and reviewed termination. The terminal bundle is lazy-loaded off the startup path. Command Builder/Intent/History/Split/Remote modes, terminal history/search/copy, SSH, structured proposals, AI suggestions, and privileged-command review remain incomplete. Steam Input/native-adapter support is still an explicit, documented gap. Routes without a real screen yet render an honest placeholder naming the screen ID and owning epic. See `HANDOFF.md` and `docs/implementation/NDX_IMPLEMENTATION_LEDGER.md` for current state and next steps.
