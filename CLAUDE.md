...
> **Epic 9 correction (2026-06-22):** Model work is no longer limited to provider connectivity. Real completion invocation, measured resource-aware routing, all eight ND-037 profiles, provider enable/disable, and capability-detected Ollama runtime controls are implemented. See `docs/implementation/NDX_EPIC_9_MODELS.md`; older deferral language below is historical.
...
**Current repository state:** Epics 0–2 complete; Epics 3, 4, 5, 6, 7, and 9 partially complete by design; the Recovery Service slice of Epic 11 is real; Epic 8's Workflow Engine is real (Agent Runtime fully deferred to Epic 9). `core/recovery/RecoveryService.ts` records a real checkpoint before every file overwrite — `FileService.write()` now exists, always checkpointed via `registerFileHandlers.ts`'s orchestration. This closed Epic 7's read-only gap: Build Studio (`/build`) has real editing and save. `/recovery` and `/storage` are real Recovery Service screens. `/automations` (ND-032 Workflow Library), `/automations/forge` (ND-033 Workflow Forge — a controller-friendly ordered step list, not a graph canvas), and `/automations/runs/:runId` (ND-034 Workflow Run Detail) are real: workflow `tool-action` steps run through the exact same Epic 4 `ActionQueue`/registry/permission/approval pipeline as a Command Palette action, `user-approval` steps genuinely pause a run, `condition`/`validator` use structured (never `eval()`) comparisons. Agent Runtime (§17) is deferred in full — an agent with no model to plan with (Epic 9) would be an empty shell. `/models` (ND-035 Model Control Center) and `/models/:providerId` (ND-036 Model Detail) are real, scoped to provider connectivity: a genuine OpenAI-compatible HTTP client (`ModelProviderService`) with a real connection test and real capability discovery against user-configured local (e.g. Ollama) or cloud endpoints; API keys are encrypted via Electron's `safeStorage` and never reach the renderer. There is no local model inference runtime, and Routing Profiles (ND-037) are deferred — they need real measured resource data (battery/thermal/memory) from a System Metrics Service that doesn't exist yet (Epic 11 §27). ND-025 Git Control Center is real for local operations plus remote fetch/pull/push/stash/conflict detection. ND-028 Direct terminal mode and ND-029 Command Builder are real with mandatory ActionQueue approval before PTY input. Git restore/discard/force-push, copy/move/rename/delete file operations, structural code edits, AI generation/intent mode, real LSP for non-TS/JS languages, AI decision/Script/Parallel/Merge/Rollback workflow node types, and the remaining 15 Epic 11 screens remain incomplete. Steam Input/native-adapter support is still an explicit, documented gap. See `HANDOFF.md` and the implementation ledger for current evidence.
