...
**Current repository state (2026-06-30):**

**Phase A (Epics 0–12): COMPLETE.** All 13 core epics are done. Validation baseline: 604+ unit/integration tests across 122+ files, 6/6 Playwright e2e scenarios, lint/typecheck/build all passing, real Linux AppImage/deb/snap built via WSL2. Version `0.1.0`. Every screen has real IPC-connected services, real controller traversal, and real loading/empty/error states — no mock production behavior anywhere in `main/` or `core/`.

**Phase B (Epics X1–X15): IN PROGRESS — all epics have real implementations; deeper enforcement, OS adapters, and pipeline features remain deferred.** Notable completed items: Flatpak/AppImage/Steam Shortcut Manager (X2); local extension host/quarantine/capability API (X3); text/JSON/CSV knowledge vault + scoped AI memory + prompt/persona/Tool Library (X4); dictation/TTS/voice notes/document intake with real mic permission gate (X5); clipboard backend, snippets backend, Share Sheet, LAN peer discovery + AES-256-GCM transfer (X6); local backup/restore/import/export/scheduled backup (X7); read-only device, Bluetooth, audio, display, storage centers (X8); resource/AI-workload/scheduler status surfaces (X9); encrypted-at-rest vault + profile/session foundations (X10); continuity/safe-mode foundations (X11); privacy data map, crash reporting, support bundles (X12); Help Hub, context help, guided troubleshooter (X13); screenshot/recording/voice notes/presentation mode/kiosk/app sandbox (X14); SBOM generation + real Ed25519 extension signing/verification (X15).

**LAN Share (Warpinator-compatible, LAN-0–LAN-11):** Core protocol complete — real gRPC/protobuf wire format, mDNS discovery, v1/v2 registration, RSA X.509 + NaCl-secretbox auth, AES-GCM chunk transfer, atomic receive/commit, and decompression-bomb + oversize-stream mitigations. 7 controller-native screens wired (LAN-7). Platform surface integrations (Activity/notifications/Search/Quick Access/AI tools) done (LAN-8). Specific-interface binding + suspend/resume real (LAN-9 partial). Real firewall/VPN detection and live binary interop against an actual Warpinator client remain deferred.

**Hybrid IDE UI (HYBRID-0–HYBRID-9):** HYBRID-0 through HYBRID-8 are complete — all routes migrated to shared workbench primitives (tool-window/editor/inspector regions, spatial lockups, settings/critical-flow framing, boot/failure primitives). HYBRID-9 (full-program visual/controller/performance evidence matrix) remains pending.

**Genuinely deferred (require named missing infrastructure before any work):** Steam Input/native adapter (rear grips L4/L5/R4/R5 inaccessible via W3C Gamepad API); OS-level suspend/reboot/shutdown; Bluetooth/audio/display/dock OS adapters (BlueZ/PipeWire/KScreen); hot-plug events; sync engine + conflict resolver; embeddings/semantic search (no embedding endpoint); signed extension marketplace + SDK/CLI; localization/IME; wake-word detection; privacy-gated screen capture; telemetry consent (no collector to consent to); durable trigger execution + quiet-hours enforcement; per-service profile isolation; replay workers; OS-level firewall assistant; real CI signing pipeline. See `HANDOFF.md` §"Risks and constraints" and `IMPLEMENTATION_CHECKLIST.md` for the complete per-item list with named reasons. See `HANDOFF.md` and `docs/implementation/NDX_IMPLEMENTATION_LEDGER.md` for full evidence.
