...
**Current repository state:** Epics 0–2 complete; Epics 3, 4, and 5 partially complete by design, and Epic 6 is active. Partial ND-025 Git Control Center and ND-028 Direct terminal mode are real. ND-029 Command Builder is now partially real at `/terminal/builder`: typed command blocks, shell-specific quoting, exact preview, deterministic risk classification, local-session targeting, copy-without-running, and mandatory registry/permission/ActionQueue approval before PTY input. Approval renders the actual command and terminal target from immutable action arguments. Context-aware option catalogs, local man pages, saved actions, remote targets, AI generation/intent mode, history/split/remote terminal modes, and richer controller text entry remain incomplete. Steam Input/native-adapter support is still an explicit, documented gap. See `HANDOFF.md` and the implementation ledger for current evidence.
