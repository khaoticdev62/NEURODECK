...
**Current repository state:** Epics 0–2 complete; Epics 3, 4, 5, and 6 partially complete by design, and Epic 7 is active. ND-025 Git Control Center is real for local operations plus remote fetch/pull/push/stash/conflict detection (tested against real bare-remote repositories — `core/git/__tests__/GitServiceRemote.test.ts`), with push always gated behind its own confirmation review separate from commit. ND-028 Direct terminal mode and ND-029 Command Builder are real at `/terminal` and `/terminal/builder` with mandatory registry/permission/ActionQueue approval before PTY input. Build Studio (`/build`) ships a real, read-only Monaco editor — locally bundled, no CDN — with real TypeScript/JavaScript diagnostics and symbol navigation from Monaco's bundled TS-compiler worker; saving is deferred until Epic 11's Recovery Service, the same rule that kept File Manager read-only in Epic 5. Restore/discard, force push, branch create/delete, AI generation/intent mode, real LSP for non-TS/JS languages, structural editing, and richer controller text entry remain incomplete. Steam Input/native-adapter support is still an explicit, documented gap. See `HANDOFF.md` and the implementation ledger for current evidence.
