...
**Current repository state:** Epics 0–2 complete; Epics 3, 4, and 5 partially complete by design, and Epic 6 is active. ND-025 Git Control Center is real for local operations plus remote fetch/pull/push/stash/conflict detection (tested against real bare-remote repositories — `core/git/__tests__/GitServiceRemote.test.ts`), with push always gated behind its own confirmation review separate from commit. ND-028 Direct terminal mode is real. ND-029 Command Builder is real at `/terminal/builder`: typed command blocks, shell-specific quoting, exact preview, deterministic risk classification, local-session targeting, copy-without-running, and mandatory registry/permission/ActionQueue approval before PTY input. Approval renders the actual command and terminal target from immutable action arguments. Restore/discard, force push, branch create/delete, context-aware option catalogs, local man pages, saved actions, remote terminal targets, AI generation/intent mode, history/split/remote terminal modes, and richer controller text entry remain incomplete. Steam Input/native-adapter support is still an explicit, documented gap. See `HANDOFF.md` and the implementation ledger for current evidence.
