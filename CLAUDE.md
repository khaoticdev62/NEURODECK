...
**Current repository state:** Epics 0–2 complete and Epic 3 partially complete (6 of 12 onboarding/global-UX screens are real and shipped; the other 6 are explicitly deferred behind documented backend dependencies — see the ledger's Epic 3 "Scope decision"). Real Gamepad API + keyboard adapters, the Spatial Focus Engine, haptics, and a dev-only focus debug overlay all exist and are tested. Real screens now live: First-Run Welcome, Controller Calibration, Home Command Center, Universal Command Palette, Activity Center, and Notification Center. Steam Input/native-adapter support (needed for Steam Deck's rear grip buttons) is an explicit, documented gap, not a missing mapping. Routes without a real screen yet render an honest placeholder naming the screen ID and owning epic. See `HANDOFF.md` and `docs/implementation/NDX_IMPLEMENTATION_LEDGER.md` for current state and next steps.
