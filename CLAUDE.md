...
**Current repository state:** Epics 0–2 complete; Epics 3, 4, 5, 6, and 7 partially complete by design; the Recovery Service slice of Epic 11 is real (the other 15 of 16 Epic 11 items are deferred). `core/recovery/RecoveryService.ts` records a real checkpoint (content snapshot + reversibility metadata, stored outside the user's workspace) before every file overwrite — `FileService.write()` now exists, the first real destructive file operation, always checkpointed via `registerFileHandlers.ts`'s orchestration (no code path skips it). This closed Epic 7's read-only gap: Build Studio (`/build`) now has real editing and real save (Ctrl+S / Save button), with a real dirty-tab indicator. `/recovery` (Recovery Timeline) and `/storage` (Storage and Recovery) are real screens for inspecting, diffing, and restoring file-write checkpoints. ND-025 Git Control Center is real for local operations plus remote fetch/pull/push/stash/conflict detection, with push always gated behind its own confirmation review separate from commit. ND-028 Direct terminal mode and ND-029 Command Builder are real at `/terminal` and `/terminal/builder` with mandatory registry/permission/ActionQueue approval before PTY input. Git restore/discard/force-push, copy/move/rename/delete file operations, structural code edits, AI generation/intent mode, real LSP for non-TS/JS languages, and the remaining 15 Epic 11 screens remain incomplete. Steam Input/native-adapter support is still an explicit, documented gap. See `HANDOFF.md` and the implementation ledger for current evidence.
