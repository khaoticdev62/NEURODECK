...
const CTRL_PROMPT_LIBRARY = [
  // ── Quick ──────────────────────────────────────────────────────────────────
  {
    ...
    fields: {
      TOPIC:
        "The subject to explain. The more specific you are — 'JWT refresh token rotation' vs 'auth' — the more precise and useful the output. You can also paste raw error text or a paragraph of docs.",
    },
    tips: [
      "Add 'like I'm 12' at the end for maximum simplicity",
      "Works on error messages, algorithm names, and documentation extracts",
      "Pair with a code block inside TOPIC for instant line-by-line walkthroughs",
    ],
  },
  {
    id: "q2",
    ...
    fields: {
      CODE: "Paste the minimal reproducible snippet. Include the function signature and any relevant imports. The smaller and more isolated the code, the more accurate the fix.",
    },
    tips: [
      "Include the error message as a comment above the code for faster diagnosis",
      "If the bug is intermittent, describe the condition that triggers it",
      "Ask for an explanation of why it broke, not just the fix",
    ],
  },
  {
    id: "q3",
    ...
    fields: {
      TEXT: "Any text: article, PR description, meeting notes, README, log output. Longer inputs produce better summaries than short ones because the compression ratio reveals what truly matters.",
    },
    tips: [
      "Change '3' to a higher number for more detail",
      "Works on git diffs — paste the diff output directly",
      "Add 'for a non-technical audience' to strip implementation details",
    ],
  },
  {
    id: "q4",
    ...
    fields: {
      CONTENT:
        "Paste anything: code, a config file, a written paragraph, a plan, a diagram description. The AI will identify the single most impactful problem without being asked for a full audit.",
    },
    tips: [
      "Great as a fast sanity check before a code review",
      "Works on written plans, not just code — useful for logic gaps in specs",
      "Follow up with 'And what else?' to get the next highest-impact issue",
    ],
  },
  {
    id: "q5",
    ...
    fields: {
      TEXT: "Any text that ends mid-thought. The last 2–3 sentences carry the most tonal weight. Including them gives the model enough context to continue seamlessly.",
    },
    tips: [
      "Works for code comments, fiction, documentation, and emails",
      "For code: end your text mid-function to have the implementation completed",
      "If the continuation drifts, prepend 'Do not change style:' to the prompt",
    ],
  },
  // ── Code ───────────────────────────────────────────────────────────────────
  {
    ...
    fields: {
      LANGUAGE:
        "The programming language. This matters — 'Python' checks for GIL issues and mutable defaults; 'Rust' checks for ownership and unsafe blocks; 'JavaScript' checks for prototype pollution and async pitfalls.",
    ...
    tips: [
      "Run this before every PR for a fast pre-flight check",
      "Append 'Focus only on security' to narrow the scope",
      "Paste the function signature + body only — avoid entire files",
    ],
  },
  {
    id: "c2",
    ...
    fields: {
      CODE: "The code to clean up. Works best on functions that have grown organically over time — long conditionals, deeply nested loops, magic numbers, and copy-pasted blocks are prime targets.",
    },
    tips: [
      "Add 'Do not extract helper functions' if you want inline refactoring only",
      "Ask it to explain each change it made for a learning opportunity",
      "Great for legacy code you inherited — pair with the Review Code prompt first",
    ],
  },
  {
    id: "c3",
    ...
    fields: {
      LANGUAGE:
        "Determines the test framework used. 'Python' → pytest fixtures; 'TypeScript' → Jest/Vitest with describe/it blocks; 'Rust' → #[cfg(test)] modules. Be specific.",
    ...
    tips: [
      "Append 'Use table-driven tests' for languages like Go to get compact parametrised coverage",
      "Ask for 'mutation testing hints' to surface what assertions are missing",
      "Pair with Review Code to fix issues before writing tests",
    ],
  },
  {
    id: "c4",
    ...
    fields: {
      CODE: "Any algorithmic function — sorting, searching, graph traversal, DP, etc. Recursive algorithms especially benefit from the step-by-step tracing. Include sample input in a comment for concrete examples.",
    },
    tips: [
      "Add 'Trace through this input: [value]' for a concrete dry-run example",
      "Ask for 'the best and worst case separately' for algorithms with variable performance",
      "Great for interview prep — follow up with 'How would you optimise this?'",
    ],
  },
  {
    id: "c5",
    ...
    fields: {
      FROM_LANG:
        "The source language. Including the version helps: 'Python 2' vs 'Python 3' produces very different output for IO and string handling.",
      TO_LANG:
        "The target language. The AI will use idiomatic patterns — e.g. converting Python list comprehensions to Go range loops, not verbose for-loops.",
    ...
    tips: [
      "Append 'and flag any idioms that don't have a direct equivalent' to catch translation losses",
      "For compiled languages, ask it to also provide a compilation command",
      "Follow up with 'Write tests for the converted code' to verify correctness",
    ],
  },
  {
    id: "c6",
    ...
    fields: {
      CODE: "Functions that currently assume all inputs are valid and all operations succeed. Network calls, file I/O, parsing functions, and database queries benefit most from this treatment.",
    },
    tips: [
      "Append 'Use the Result/Either pattern' for functional-style error handling in supported languages",
      "Ask for 'user-facing error messages' separately from internal error logging",
      "Follow up with 'Write tests for the error paths' to ensure the handling actually works",
    ],
  },
  {
    id: "c7",
    ...
    fields: {
      CODE: "Performance-sensitive code — hot paths, inner loops, data processing pipelines, or functions called thousands of times. Include comments about what the bottleneck symptoms are if you know them.",
    },
    tips: [
      "Specify the bottleneck dimension: 'optimise for latency' vs 'optimise for memory' vs 'optimise for throughput'",
      "Append 'Do not change the public interface' to keep callers intact",
      "Profile before optimising — paste benchmark results so the AI targets the right function",
    ],
  },
  {
    id: "c8",
    ...
    fields: {
      LANGUAGE:
        "Picks the documentation format: 'TypeScript' → JSDoc with @param/@returns; 'Python' → Google-style or NumPy docstrings; 'Rust' → /// rustdoc comments with # Examples sections.",
    ...
    tips: [
      "Append 'Include a usage example in each docstring' for developer-facing APIs",
      "Ask for 'README-style documentation' separately from inline comments for open-source projects",
      "Specify 'terse comments only — no stating the obvious' to avoid comment noise",
    ],
  },
  // ── Analysis ───────────────────────────────────────────────────────────────
  {
    ...
    fields: {
      TOPIC:
        "Any complex subject: a system architecture, a business concept, a technical protocol, an algorithm, or a framework. Paste a block of text or just name the topic — both work.",
    },
    tips: [
      "Append 'and how each component interacts with the others' to get the integration map too",
      "Great for onboarding onto unfamiliar codebases — paste a module list or file tree",
      "Follow up with 'Which component is the most critical single point of failure?' for risk analysis",
    ],
  },
  {
    id: "a2",
    ...
    fields: {
      TOPIC:
        "A technology, architectural decision, business strategy, or any binary-ish choice. Include context ('for a team of 3 building a real-time app') to get contextualised trade-offs rather than abstract ones.",
    },
    tips: [
      "Add 'in the context of [your situation]' to get relevant trade-offs, not generic ones",
      "Ask for 'the one pro and one con that matter most' for a quick decision shortcut",
      "Pair with Compare Options when evaluating two specific alternatives head-to-head",
    ],
  },
  {
    id: "a3",
    ...
    fields: {
      PROBLEM:
        "Describe the observed symptom AND the context: when it happens, what changed recently, what you've already tried. The more operational context you provide, the deeper the AI can drill.",
    },
    tips: [
      "Append 'Give me the top 3 most likely root causes ranked by probability' for a prioritised list",
      "Include error logs, metric graphs descriptions, or timeline of events for richer analysis",
      "Follow up with 'For each cause, what is the fastest way to confirm or rule it out?' to get a triage plan",
    ],
  },
  {
    id: "a4",
    ...
    fields: {
      OPTION_A:
        "First option to compare. Be specific — 'PostgreSQL' beats 'a relational database'. The more precise the name, the more accurate the trade-off analysis.",
      OPTION_B:
        "Second option to compare. Ideally a direct alternative to OPTION_A at the same abstraction level — don't compare a language to a framework.",
    },
    tips: [
      "Add more dimensions: 'also compare on: learning curve, ecosystem maturity, vendor lock-in'",
      "Append 'For a startup with 2 engineers and 10k MAU' to get context-specific recommendations",
      "Follow up with 'Which would you recommend and why?' for a decisive conclusion",
    ],
  },
  {
    id: "a5",
    ...
    fields: {
      CONTENT:
        "Code, architecture description, API spec, config file, or infrastructure diagram. Include auth flows, data flows, and trust boundaries explicitly — they're where most vulnerabilities live.",
    },
    tips: [
      "Append 'Use OWASP Top 10 as the evaluation framework' for standardised output",
      "Ask for severity ratings (Critical/High/Medium/Low) for each finding",
      "Follow up with 'Write the unit tests that would catch these vulnerabilities'",
    ],
  },
  {
    id: "a6",
    ...
    fields: {
      DESCRIPTION:
        "Describe the system: components, how they connect, data flows, and expected load characteristics. A numbered component list works well. Include current pain points if any — they often point directly to the architecture's weaknesses.",
    },
    tips: [
      "Append 'Assume we need to handle 10x current load within 6 months' to get forward-looking feedback",
      "Ask for 'the single most impactful change' first, then iterate",
      "Include deployment topology (cloud provider, region count, CDN usage) for infrastructure-specific advice",
    ],
  },
  // ── Creative ───────────────────────────────────────────────────────────────
  {
    ...
    fields: {
      GENRE:
        "Sets the aesthetic rules and reader expectations — 'cyberpunk' implies neon, corporate dystopia, and hacking; 'cozy mystery' implies small town, amateur sleuth, and gentle stakes. Be specific for tighter output.",
      CHARACTER:
        "The protagonist. Give them a defining trait or contradiction: 'a retired assassin who can't stop helping people'. One-dimensional characters produce flat stories.",
      CHALLENGE:
        "The dramatic engine — what the character must overcome. 'Stop a server from going down' is more specific and interesting than 'solve a problem'. Include stakes if possible.",
    },
    tips: [
      "Add 'in 500 words' for a short-form output suitable for sharing",
      "Ask for the story 'in the style of [author]' to match a specific voice",
      "Follow up with 'Write the next scene' to extend the story iteratively",
    ],
  },
  {
    id: "cr2",
    ...
    fields: {
      TOPIC:
        "The domain to brainstorm in. The more constrained your topic, the more creative the ideas — 'marketing for a solo developer B2B SaaS' produces better ideas than 'marketing ideas'.",
    },
    tips: [
      "Ask for ideas across multiple dimensions: 'include 3 low-cost, 3 high-impact, and 4 wild-card ideas'",
      "Follow up with 'Take idea #5 and expand it into a full plan'",
      "Append 'that no one has tried before in this space' to maximally suppress generic suggestions",
    ],
  },
  {
    id: "cr3",
    ...
    fields: {
      PRODUCT:
        "The product or feature being marketed. Include the core value proposition in 1 sentence: 'A terminal app for Steam Deck that lets you chat with AI while gaming'.",
      AUDIENCE:
        "Who the copy is written for. The more specific the better: 'solo indie hackers who ship fast and hate marketing' hits harder than 'developers'.",
    ...
    tips: [
      "Ask for multiple variations: 'Write 3 versions with different hooks'",
      "Append 'Include a clear CTA' to ensure the copy drives action",
      "Ask for a tagline, a tweet, and a landing page paragraph as separate deliverables",
    ],
  },
  {
    id: "cr4",
    ...
    fields: {
      TEXT: "Any writing: email, README intro, pitch paragraph, blog post section, or code comment. The AI improves sentence rhythm, removes filler words, strengthens verbs, and adds clarity without changing your intent.",
    },
    tips: [
      "Add 'Do not change the first sentence' to preserve your opening hook",
      "Ask for 'a tracked-changes version' to see exactly what was altered and why",
      "Follow up with 'Now make it 30% shorter without losing meaning' for tight editing",
    ],
  },
  {
    id: "cr5",
    ...
    fields: {
      STYLE:
        "The formal constraint: 'haiku' (17 syllables, seasonal reference), 'Shakespearean sonnet' (14 lines, ABAB rhyme), 'free verse' (no formal constraints), 'limerick' (humorous AABBA). Form shapes meaning.",
      TOPIC:
        "The subject or emotion. Abstract topics work well in poetry: 'the frustration of a bug you can't find', 'the feeling of a clean git history'. Specificity creates universality in poetry.",
    },
    tips: [
      "Ask for an annotation explaining the imagery choices for a learning exercise",
      "Request 'a title that isn't obvious from the content' for more sophisticated output",
      "Append 'Do not use clichés' to push past 'time flies' and 'broken wings'",
    ],
  },
  // ── Technical ─────────────────────────────────────────────────────────────
  {
    ...
    fields: {
      SYSTEM:
        "What you're building: include scale expectations, team size, and key constraints (latency requirements, budget, existing stack). 'Real-time collaborative editor for 10k concurrent users, team of 2' produces a targeted design.' ",
    },
    tips: [
      "Append 'Focus on what a single developer could build in 3 months' for realistic scoping",
      "Ask for 'the MVP version and the v2 version separately'",
      "Request a numbered component list first, then expand each one individually",
    ],
  },
  {
    id: "t2",
    ...
    fields: {
      SERVICE:
        "The service the API exposes. Be specific about the entities involved: 'A task management service with users, projects, and tasks. Tasks can have subtasks and file attachments.' This level of detail drives the schema design.",
    },
    tips: [
      "Append 'Follow REST best practices including versioning, pagination, and error response format'",
      "Ask for OpenAPI/Swagger YAML output for immediate tooling compatibility",
      "Follow up with 'Write the rate limiting strategy and abuse prevention headers'",
    ],
  },
  {
    id: "t3",
    ...
    fields: {
      APPLICATION:
        "Describe the application's data model: the entities, their relationships, and the queries you need to optimise for. 'A multi-tenant SaaS where each tenant has users, who create projects, which have tasks with comments and file attachments' gives enough detail.",
    },
    tips: [
      "Append 'Assume PostgreSQL — use appropriate types including JSONB and arrays where useful'",
      "Ask for 'the migration SQL to create this schema' for immediate implementation",
      "Follow up with 'What indexes would I add for the 5 most common query patterns?'",
    ],
  },
  {
    id: "t4",
    ...
    fields: {
      APPLICATION:
        "What you're deploying: language, framework, stateful components (databases, queues, caches), and approximate traffic. Stateful components dramatically change the deployment strategy.",
      ENVIRONMENT:
        "The target platform: 'AWS ECS Fargate', 'bare metal Ubuntu VPS', 'Kubernetes on GKE', 'Vercel + PlanetScale'. This determines which services, tools, and patterns are relevant.",
    },
    tips: [
      "Append 'Optimise for a solo developer — minimise operational overhead'",
      "Ask for 'the deployment checklist as a numbered runbook'",
      "Include 'zero-downtime requirement' if your service must stay available during deploys",
    ],
  },
  {
    id: "t5",
    ...
    fields: {
      DESCRIPTION:
        "Describe the system's architecture, the observed performance symptoms (p99 latency, throughput limits, memory pressure), and what you've already tried. Include approximate data volumes and request rates — bottleneck analysis is meaningless without scale context.",
    },
    tips: [
      "Paste actual benchmark numbers or profiler output for the most targeted advice",
      "Ask for bottlenecks ranked by estimated impact (High/Medium/Low)",
      "Append 'Suggest what to instrument/measure first to validate each hypothesis'",
    ],
  },
  {
    id: "t6",
    ...
    fields: {
      PROJECT_TYPE:
        "What kind of product: 'real-time collaborative SaaS', 'static marketing site', 'high-throughput data pipeline', 'mobile app with offline support'. The architecture pattern flows from the type.",
      TEAM_SIZE:
        "Number of engineers and their backgrounds: 'solo TypeScript developer', 'team of 4 with Python/ML background'. Stack complexity must match team capacity to maintain it.",
      SCALE:
        "Expected user load and data volume: 'MVP for 100 users', '50k DAU with 1TB data', '10M events/day'. Over-engineering for scale you won't reach is as damaging as under-engineering.",
    },
    tips: [
      "Append 'Bias toward boring technology that is well-understood' for lower operational risk",
      "Ask for 'what to start with vs what to migrate to at 10x scale' for a growth path",
      "Include 'No VC funding — bootstrapped' to filter out enterprise-priced solutions",
    ],
  },
  // ── Roleplay ──────────────────────────────────────────────────────────────
  {
    ...
    fields: {
      APPROACH:
        "Describe your technical plan, design decision, or implementation. Include the trade-offs you already see — the AI's feedback is most valuable when it challenges your assumptions, not just validates your concerns.",
    },
    tips: [
      "Add 'Do not soften the feedback' for maximum directness",
      "Ask for 'what the biggest mistake junior engineers make in this situation'",
      "Follow up with 'What would you do differently and why?'",
    ],
  },
  {
    id: "r2",
    ...
    fields: {
      TOPIC:
        "The concept you want to genuinely understand, not just be told about. Works best for foundational topics where you have some intuition but haven't fully grasped the principle: 'how TCP/IP actually works', 'why React re-renders', 'the CAP theorem'.",
    },
    tips: [
      "This is slow but deep — use it when you want to truly understand something, not when you need a quick answer",
      "If you get stuck, say 'I don't know' — the AI will adjust its questions rather than just give the answer",
      "Follow up across multiple sessions to build cumulative understanding",
    ],
  },
  {
    id: "r3",
    ...
    fields: {
      POSITION:
        "A decision, belief, or plan you're about to act on. The more committed you feel, the more valuable this exercise is — it's hardest to find flaws in things you're excited about, which is exactly when you need to look hardest.",
    },
    tips: [
      "Use this before any major technical decision — architecture choices, tech stack selections, build vs buy",
      "Follow up with 'Now steelman my original position' to rebuild confidence in the parts that survived scrutiny",
      "Ask for 'the one counter-argument that would make you reverse the decision'",
    ],
  },
  {
    id: "r4",
    ...
    fields: {
      CODE: "The code you're about to merge or ship. The AI reviews for correctness, maintainability, style, naming, error handling, and hidden complexity. Include the PR description or the problem it solves for context.",
    },
    tips: [
      "Add 'Grade it on a scale of 1–10 and justify the score' for a quantified quality signal",
      "Ask for 'the one change that would have the highest impact on code quality'",
      "Follow up with 'Now rewrite the worst function the way you would write it'",
    ],
  },
  {
    id: "r5",
    ...
    fields: {
      FEATURE:
        "The feature request or idea. Include who asked for it, how many users it affects, and what problem it solves. If you have data (support tickets, user research), include it — the AI's impact estimate is only as good as the evidence you provide.",
    },
    tips: [
      "Add 'Suggest the smallest version of this feature that delivers the core value' for an MVP scoping",
      "Ask for 'what metric would prove this feature succeeded or failed'",
      "Follow up with 'What features should we NOT build this sprint?' for prioritisation pressure",
    ],
  },
];
...
function getCtrlPromptUsage() {
  try {
    return JSON.parse(localStorage.getItem("ctrlPromptUsage") || "{}");
  } catch {
    return {};
  }
}
...
function filterCtrlPrompts(query, catId) {
  ...
  let pool =
    catId === "custom"
      ...
      : catId === "all"
        ? CTRL_PROMPT_LIBRARY
        : CTRL_PROMPT_LIBRARY.filter((p) => p.cat === catId);
  if (q) {
    pool = pool.filter(
      (p) =>
...
function getCustomCtrlPrompts() {
  try {
    return JSON.parse(localStorage.getItem("ctrlCustomPrompts") || "[]");
  } catch {
    return [];
  }
}
...
function renderCtrlPromptCats() {
  ...
  if (!container) return;
  const allCats = [
    { id: "all", icon: "fileText", label: "All", color: CAT_COLORS.all },
    ...CTRL_PROMPT_CATS,
  ];
...
function _buildCtrlPromptRow(p, i, uses, usePct, isFocused) {
  ...
  const templateBadge = hasTemplate
    ? ` <span class="ctrl-prompt-template-badge" style="background:${color}22;color:${color};">template</span>`
    : "";
...
function renderCtrlPromptList() {
  ...
  container.innerHTML = ctrlPromptFiltered
    .map((p, i) => {
...
function updateCtrlPromptPreview(prompt) {
  ...
  if (prompt.fields && Object.keys(prompt.fields).length > 0) {
    const rows = Object.entries(prompt.fields)
      .map(
        ([name, explanation]) =>
...
function navigateCtrlPromptList(delta) {
  ...
  ctrlPromptListIdx =
    (ctrlPromptListIdx + delta + ctrlPromptFiltered.length) % ctrlPromptFiltered.length;
  renderCtrlPromptList();
}
...
function parseTemplateText(text) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    parts.push({
      ...
      name: m[1],
      value: m[1],
      options: getPlaceholderOptions(m[1]),
      optIdx: 0,
    });
...
function getTemplateFilled() {
  return ctrlPromptTemplateParts
    .map((p) => {
...
function navigateTemplatePlaceholder(delta) {
  ...
  ctrlPromptPlaceholderIdx =
    (ctrlPromptPlaceholderIdx + delta + placeholders.length) % placeholders.length;
  renderTemplateModeFooter();
}
...
function initCtrlPromptPicker() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      ctrlPromptTemplateMode ? navigateTemplatePlaceholder(1) : navigateCtrlPromptList(1);
    }
    if (e.key === "ArrowUp") {
      e.preventDefault();
      ctrlPromptTemplateMode ? navigateTemplatePlaceholder(-1) : navigateCtrlPromptList(-1);
    ...
    if (e.key === "ArrowLeft") {
      e.preventDefault();
      ctrlPromptTemplateMode ? cycleTemplatePlaceholder(-1) : navigateCtrlPromptCat(-1);
    }
    if (e.key === "ArrowRight") {
      e.preventDefault();
      ctrlPromptTemplateMode ? cycleTemplatePlaceholder(1) : navigateCtrlPromptCat(1);
    ...
    if (e.key === "Enter") {
      e.preventDefault();
      ctrlPromptTemplateMode ? confirmTemplateAndSend() : confirmCtrlPrompt();
    }
  });
...
const PANEL_TEMPLATES = {
  chat: [
    {
      label: "Summarize",
      text: "Summarize the key points of the following in 3 bullet points:\n\n{{input}}",
    },
    ...
    {
      label: "Rewrite",
      text: "Rewrite the following to be clearer and more concise:\n\n{{input}}",
    },
  ...
  terminal: [
    ...
    {
      label: "Memory info",
      text: "free -h && cat /proc/meminfo | grep -E 'MemTotal|MemFree|MemAvailable'",
    },
  ...
  agent: [
    {
      label: "Write script",
      text: "Write a Python script that {{input}}. Include error handling and a usage example.",
    },
    ...
    {
      label: "API wrapper",
      text: "Write a Python wrapper for the {{input}} API with error handling and retries.",
    },
    {
      label: "Data pipeline",
      text: "Create a data pipeline that reads from {{input}}, transforms, and writes to CSV.",
    },
    { label: "Config gen", text: "Generate a production-ready config file for {{input}}." },
    {
      label: "Perf analyze",
      text: "Analyze performance bottlenecks and suggest optimizations for:\n\n{{input}}",
    },
    { label: "Security audit", text: "Perform a security audit of:\n\n{{input}}" },
    {
      label: "Deploy script",
      text: "Write a deployment script for {{input}} that handles rollback.",
    },
  ],
  canvas: [
    {
    ...
    {
      label: "Fetch API",
      text: "const res = await fetch('{{input}}');\nconst data = await res.json();\nconsole.log(data);",
    },
    {
      label: "CSS variables",
      text: ":root {\n  --primary: #00e5ff;\n  --bg: #0d0d14;\n  --text: #e0e0f0;\n}",
    ...
    {
      label: "Tailwind card",
      text: '<div class="rounded-xl border border-zinc-700 bg-zinc-900 p-6">\n  <h2 class="text-lg font-bold">{{input}}</h2>\n</div>',
    },
    {
      label: "SQL query",
    ...
    {
      label: "Lua plugin",
      text: "registerCommand('{{input}}', function(args)\n  return execute(args)\nend)",
    },
    {
      label: "JSON schema",
...
function _pasteTemplate(view, text) {
  ...
  if (view === "terminal") {
    ...
    if (ptyInput) {
      ptyInput.dispatchEvent(new InputEvent("input", { data: clean, bubbles: true }));
    }
    return;
  }
  // Agent
  ...
  if (view === "canvas") {
    if (window._monacoEditor) {
      window._monacoEditor.setValue(full);
      return;
    }
...
export function getCtrlPromptVisible() {
  return ctrlPromptVisible;
}
export function getCtrlPromptTemplateMode() {
  return ctrlPromptTemplateMode;
}
