...
function _rcWireClientEvents(rc) {
  ...
  listen("remote_chat", function (ev) {
    ...
    const chatInput =
      document.getElementById("chat-input") || document.getElementById("user-input");
    if (chatInput) {
      chatInput.value = text;
      const sendBtn =
        document.getElementById("send-btn") || document.getElementById("chat-send-btn");
...
function _rcWireControlEvents(rc) {
  ...
  listen("remote_persona_cycle", function (ev) {
    ...
    const current = arr.findIndex(function (i) {
  ...
  listen("remote_theme_cycle", function () {
    const btn =
      document.getElementById("theme-cycle-btn") ||
      document.querySelector('[data-action="cycle-theme"]');
    if (btn) btn.click();
  });
  listen("remote_open_settings", function () {
    const btn =
      document.getElementById("settings-btn") || document.querySelector(".settings-toggle-btn");
    if (btn) btn.click();
  });
