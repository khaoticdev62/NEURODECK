...
function performSearch(query) {
  ...
  state.chatMessageRegistry.forEach((entry) => {
    ...
    while ((node = walker.nextNode())) {
      if (node.parentElement.closest("button, .msg-meta, .msg-copy-btn, .msg-actions-menu"))
        continue;
      textNodes.push(node);
    }
...
function forkFromMessage(msgEntry) {
  ...
  invoke("fork_session", { baseMessages })
    .then((newId) => {
      loadSession(newId);
      if (typeof addNotification === "function") {
...
function editMessageInPlace(msgEntry) {
  ...
  const doSave = () => {
    ...
    if (t) _editDoFork(t, msgEntry, cleanup);
    else cleanup();
  };
...
function _sendNormalMode(text, imageAttachment) {
  ...
  if (imageAttachment) {
    invokeArgs.image_base64 = imageAttachment.data;
    invokeArgs.image_mime = imageAttachment.mime;
  }
  if (state.activeAgentId) {
    invokeArgs.agent_id = state.activeAgentId;
  }
...
function sendMessage() {
  ...
  if (state.comparisonMode) {
    _sendComparisonMode(text, imageAttachment);
    return;
  }
...
function updateInputConsoleState() {
  ...
  if (state.isProcessRunning) {
    userInput.placeholder =
      "[Terminal Executing...] Type input for process and press Enter (Ctrl+C to Terminate)";
...
function sendProcessInput() {
  ...
  invoke("write_to_process", { input: text })
    .then(() => {
...
async function _execCodeBlock(execBtn, pre, code, lang) {
  const confirmed = await window.showConfirm(`Execute this ${lang} snippet?`, {
    confirmText: "Execute",
    cancelText: "Cancel",
  });
  if (!confirmed) return;
...
async function runLuaScript(scriptCode, preElement, execBtn) {
  ...
  const confirmed = await window.showConfirm("Execute this Lua script?", {
    confirmText: "Execute",
    cancelText: "Cancel",
  });
  if (!confirmed) return;
...
listen("stream_done", function () {
  ...
  if (state.currentAIMessage) {
    ...
    if (typeof window.announceToScreenReader === "function") {
      ...
      window.announceToScreenReader(
        `Response received. ${preview}${state.currentAIText.length > 120 ? "..." : ""}`
      );
    }
  }
  ...
  if (
    ...
    _ttsMode === "complete" &&
    state.currentAIText &&
    state.currentAIText.trim().length > 0
  ) {
...
listen("compare_stream_done", function (event) {
  ...
  const leftDone =
    !state.compareLeft.currentAIMessage ||
    !state.compareLeft.currentAIMessage.classList.contains("thinking");
  const rightDone =
    !state.compareRight.currentAIMessage ||
    !state.compareRight.currentAIMessage.classList.contains("thinking");
  ...
  if (!state.isRecording) {
    ...
    invoke("start_recording")
      ...
      .catch((err) => {
        ...
        appendChatMessage("system", `System error starting recording: ${String(err)}`, {
          error: true,
        });
      });
  } else {
...
function _buildSessionItem(sid) {
  ...
  deleteBtn.onclick = async (e) => {
    e.stopPropagation();
    const confirmed = await showConfirm(`Delete session ${sid}?`, {
      confirmText: "Delete",
      cancelText: "Keep",
    });
    if (confirmed)
      invoke("delete_session", { id: sid }).then(() => {
        if (sid === state.currentSessionId) startNewSession();
...
function loadSession(sid) {
  invoke("load_session_by_id", { id: sid })
    .then((data) => {
...
function _chatKeyCtrlAction(e) {
  ...
  if (e.ctrlKey && e.key === "b") {
    ...
    if (state.pendingLuaScript) {
      ...
      appendChatMessage("system", "System: No pending Lua script found in chat to execute.", {
        error: true,
      });
    }
  }
  ...
  if (e.ctrlKey && e.key === "p") {
    ...
    if (state.availablePersonas.length > 0) {
      ...
      let nextPersona =
        state.availablePersonas[(currentIndex + 1) % state.availablePersonas.length];
...
function _chatWireBasicButtons() {
  const sendBtn = document.getElementById("send-btn");
  if (sendBtn) sendBtn.onclick = handleSendAction;
  if (micBtn) micBtn.onclick = handleMicAction;
  const newChatBtn = document.getElementById("new-chat-btn");
  if (newChatBtn) newChatBtn.onclick = startNewSession;
  const newChatBtnHeader = document.getElementById("new-chat-btn-header");
  if (newChatBtnHeader) newChatBtnHeader.onclick = startNewSession;
  const compareToggleBtn = document.getElementById("compare-toggle-btn");
  if (compareToggleBtn) compareToggleBtn.onclick = toggleComparisonMode;
  const personaSelect = document.getElementById("persona-select");
  if (personaSelect) personaSelect.onchange = handlePersonaChange;
  const themeSelect = document.getElementById("theme-select");
  if (themeSelect) themeSelect.onchange = handleThemeChange;
...
function _chatWireExportDropdown() {
  ...
  exportMenu.querySelectorAll(".chat-export-item").forEach((item) => {
    item.addEventListener("click", async () => {
      ...
      try {
        const sessionId =
          document.getElementById("chat-session-name")?.dataset?.sessionId ||
        ...
        addNotification(
          "Exported",
          `Session copied as ${fmt.toUpperCase()} to clipboard.`,
          "success"
        );
      } catch (err) {
