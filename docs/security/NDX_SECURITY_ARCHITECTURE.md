...
- This remains a partial security boundary: richer per-run audit tabs are not implemented yet. Do not add a separate agent-only executor; the ActionQueue bridge is the only allowed tool path.
...
## 25. File Service delete security (Epic 5 addendum)

- **Delete never bypasses Recovery.** `registerFileHandlers.ts`'s `fileDelete` channel reads the file's current content via `FileService.readIfExists()` and calls `RecoveryService.recordCheckpoint()` (kind: `file-delete`) before calling `FileService.delete()` — the identical orchestration `fileWrite` and `gitRestore` already use, extended to a third real checkpoint kind rather than forked into a separate mechanism.
- **Directories cannot be deleted through this path.** `FileService.delete()` resolves the path the same way `read()` does and reuses its directory check — a directory delete attempt throws before any checkpoint is recorded or any filesystem mutation happens, the same path-traversal protection (`fs.realpath`-based, catches symlink escapes) every other `FileService` method already enforces.
- **The Delete control gates behind a `ConfirmationDialog`** naming the exact file and stating the real recovery guarantee (a checkpoint exists, restorable from Recovery Timeline) rather than implying an unqualified "undo."

## 26. AI-safety tool capability trust model (Epic 8 addendum — `files-write`/`files-delete`)

Adding the first real tools capable of mutating workspace content to the shared `ToolRegistry` (`ai-safety/tools/fileTools.ts`) required confirming the trust model explicitly, since the registry is consumed by two different callers with different threat profiles:

- **Workflow Forge tool-action steps**: the `toolId`/`args` JSON is typed by the human author at design time (Workflow Forge has no model in the loop — see its own scope note in `WorkflowForge.tsx`). There is no untrusted input here at all; this is the user configuring their own automation.
- **Agent Runtime tool calls**: `toolId`/`args` come from strict model-emitted JSON, but a tool is only reachable if a human first added its capability to that specific agent's `toolAllowlist`/`permissionCeiling` when configuring the agent. This is the same boundary `terminalCommandTools.ts` already relies on — an agent granted `terminal.execute` can already run an arbitrary shell command in any session it specifies; granting `files.write`/`files.delete` is not a new category of risk, it is the same risk shape (human-granted capability + model-chosen arguments within that capability) the codebase already accepts for terminal tools.
- **The underlying service still enforces its own boundary regardless of caller**: `FileService`'s real path-traversal protection (`fs.realpath`-based, catches symlink escapes) means neither a human-typed workflow argument nor a model-emitted one can write outside the named workspace's root, no matter what `relativePath` is supplied.
- **`PermissionBroker.evaluate()` required no special-casing** — `files.write`/`files.delete` default to `requires-approval` exactly like every other capability with no standing grant; nothing about this addition bypasses the existing approval gate.
