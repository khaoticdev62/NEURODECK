...
### Addendum — ND-044 Structured CSS Theme Customization Engine (Phase 1 of VS Code/tvOS hybrid, theming, Decky, chat roadmap)

Extended ND-044 from three overrides (reduce-motion/high-contrast/text-scale) to a real structured theme builder — accent color, corner radius, spacing density, surface style, and focus-indicator style — following the exact same pattern the original three fields established rather than inventing a new mechanism: `shared/contracts/displaySettings.ts` (schema, now with `accentSchema`/`radiusStyleSchema`/`densitySchema`/`surfaceStyleSchema`/`focusStyleSchema`, each `.default()`-ed for forward compatibility) → `DisplaySettingsStore` (JsonStore-backed, `DEFAULT_SETTINGS` extended) → `registerDisplaySettingsHandlers.ts` (unchanged — `displaySettingsSchema.safeParse` already validates whatever the schema defines) → `state/displaySettings.tsx`/`displaySettingsContext.ts`/`useDisplaySettings.ts` (five new setters mirroring `setReduceMotion`'s shape) → `DisplayThemeSettings.tsx` (five new sections) → `tokens.css` (new `[data-ndx-accent]`/`[data-ndx-radius]`/`[data-ndx-density]`/`[data-ndx-surface]`/`[data-ndx-focus-style]` attribute blocks) → `NdxWorkbench.tsx`/`ShellLayout.tsx` (attributes threaded onto the workbench root, replacing a previously hardcoded `data-ndx-theme="hybrid-dark"` literal with a named `NDX_THEME` constant).

Real findings that shaped the implementation: `JsonStore.read()` does a bare `JSON.parse` with no default-merge, so a settings file persisted before this change would parse successfully but be missing the five new keys — `DisplaySettingsStore.get()` now merges over `DEFAULT_SETTINGS` to stay forward-compatible, covered by a dedicated test that writes an old-shape JSON file directly and asserts the read-back result. The accent palette deliberately reuses the four existing, already-contrast-checked status hues (`--color-status-info/-approval/-warning/-success/-error`) plus the original cyan default rather than introducing new, unvetted colors — a curated set of five, not a free color picker, per the user's explicit scope choice. Two focus-indicator indirection points (`--ndx-focus-indicator-surface`, `--ndx-focus-indicator-row`) were added so `[data-ndx-focus-style]` only has to repoint two variables; the existing 'dense'/'spatial' focus-density shadows on `.ndx-focus-surface` were deliberately left alone since those encode structural context (list row vs. floating card), not the user's style preference. Density scaling intentionally excludes `--ndx-button-height`/`--ndx-target-min` — shrinking interactive targets below the 48px accessibility minimum for a "compact" preference would violate the wireframe spec's touch-target rule. Glass surface mode is gated by `[data-ndx-surface='glass'][data-high-contrast='false']` in CSS and the corresponding control is disabled in the settings UI whenever high contrast is on, since a translucent/blurred surface would undermine the separation high contrast exists to provide.

This is Phase 1 of a four-phase roadmap (tvOS-style navigation reskin, a Claude-Desktop-style multi-turn AI chat surface, and a real Decky Loader plugin + local control-plane bridge are Phases 2–4, tracked separately, not started).

`DisplaySettingsStore.test.ts` (+1: old-shape-file forward-compat), `DisplayThemeSettings.test.tsx` (+5: accent/radius/density/focus-style persistence + glass-disabled-under-high-contrast), `displaySettingsClient.test.ts` (content updated to the full 8-field shape, count unchanged), plus one new Playwright e2e assertion (`e2e/app.spec.ts`) that changing these controls actually patches the live `data-ndx-*` attributes on the workbench root — the only check that would catch a prop-threading regression between `DisplayThemeSettings`, `ShellLayout`, and `NdxWorkbench`.

```text
npm run typecheck    → 0 errors
npm run lint          → 0 errors, 0 warnings (10548 pre-existing CRLF prettier/prettier warnings repo-wide, unrelated to this change)
npm run test          → 247 files, 1209 of 1210 tests passed (1 confirmed-flaky pre-existing test, GuidedControllerTutorial.test.tsx — a waitFor timeout under parallel-suite load, passes cleanly in isolation; unrelated to this change)
npm run build         → succeeded
...
## Epic X14 — Recording (2026-06-29)

Real screen recording (supplemental spec §42.2) — the first `MediaRecorder`-based capture anywhere in this codebase (even Epic X5's Voice Notes capture path was never wired to a live `MediaRecorder`). Scoped deliberately after a user check-in on risk/size: full real recording, system-audio deferred, no Activity-task integration.

**`RecordingService`** (`src/main/recording/RecordingService.ts`, new) — lives in `main/`, matching `ScreenshotService`'s precedent for keeping `electron` imports (`desktopCapturer`) out of `core/`. `listSources()` returns real screen/window sources with real `NativeImage.toDataURL()` thumbnails. `begin()`/`appendChunk()`/`finish()`/`cancel()` manage a real, incremental `fs.createWriteStream` per open recording — chunks arrive as base64 from the renderer and are written immediately, never buffered whole in memory on either side, the same principle `ScreenshotService`'s single-frame capture didn't need but a multi-minute video does. A JSON sidecar file (`<video>.webm.json`) holds the real finished-recording metadata; `list()` skips any sidecar that's missing or corrupt (an interrupted recording) rather than failing the whole list.

**Why capture happens in the renderer, not main**: Electron's own documented constraint — `chromeMediaSource: 'desktop'` capture via `getUserMedia` only works in a renderer's web platform APIs; the main process can only enumerate real sources via `desktopCapturer`, never capture them itself.

**`RecordingProvider`** (`src/renderer/src/state/recording.tsx`, new) — owns the real `MediaStream`/`MediaRecorder` lifecycle. `startRecording()` requests a real desktop-capture video stream via `buildDesktopCaptureConstraints()` (Electron's real, non-standard `mandatory`/`chromeMediaSourceId` shape), optionally requests a real separate microphone `getUserMedia({ audio: true })` stream and combines its audio track with the video track into one `MediaStream` — the microphone request goes through the exact same already-real `'media'` permission handler Epic X5 established (`microphonePermissionStore.isGranted()`), unmodified. `MediaRecorder.ondataavailable` fires on a real 2-second timeslice; each chunk is converted to base64 via `blobToBase64()` (pure browser APIs only — the renderer has no Node `Buffer`) and sent to main immediately. A real `F9` keyboard shortcut (module-level constant, matching the hotkey convention real screen recorders like OBS already use) stops an active recording from anywhere in the app. `RecordingIndicatorOverlay` is rendered as a sibling of `children` inside the provider itself (the same self-contained pattern `ToastProvider`/`ToastHost` already established), so a persistent "Recording · MM:SS" banner with a Stop button stays visible across every route while recording.

**`/recording` screen** (ND-X058 — the same screen id as Screenshot Center, since the spec's own ND-X0xx table combines "Screenshot and Recording Center" into one entry; this implementation splits them into two real, separately-routed screens for clarity, the same kind of pragmatic split/fold other phases this session already used) — real source picker, 720p/1080p/native resolution presets, 15/30/60fps, microphone toggle, and "Hide secrets while recording" which calls the already-real `usePresentationMode().setPresentationMode()` rather than building a second privacy-exclusion mechanism — remembers whether Presentation Mode was already on before recording started so stopping never silently disables a mode the user had deliberately turned on themselves. Storage estimate is a real, clearly-labeled bitrate-based estimate (`estimatedBytesPerSecond()`) before/during recording, replaced by the real measured byte count from the finished `RecordingRecord` once a recording actually completes. "Share" reuses the same `navigate('/lan-share/send', { state: { sourcePaths: [...] } })` handoff `ScreenshotCenter`/`FileManager` already established.

**Honestly deferred, each named**: system-audio capture (Chromium/Electron's loopback audio support is OS-dependent and unreliable on Linux without assuming a specific PulseAudio/PipeWire monitor-source configuration this app cannot assume exists), Activity-task integration (no real Activity Center backing model exists — confirmed during the earlier Privacy and Data Map pass, where "Audit logs" turned out to be renderer-in-memory-only with no broader Activity entity), Selected region/Current panel/Annotation/Redaction (same structural reasons already named for Screenshot Center — no real selection-overlay or drawing-canvas UI exists).

**Testing**: `RecordingService` itself has no unit test — confirmed by direct experimentation that importing a module which does `import { desktopCapturer } from 'electron'` fails to resolve that named export when Vitest runs it in a plain Node process (the `electron` npm package resolves to a binary path string outside a real Electron process, not a module with named exports), the exact same constraint that already kept `ScreenshotService` untested. Real coverage instead comes from: `recordingMedia.test.ts` (6 tests — pure helpers: resolution-to-dimension mapping, the real Electron capture-constraints shape, a real binary-to-base64 round trip using Node's native `Blob` to work around jsdom's `Blob` lacking `arrayBuffer()`, and the bitrate estimate), `recording.test.tsx` (2 tests — the provider's full real lifecycle: start sends a real `begin` IPC call and streams a real chunk through to `appendChunk` the instant `MediaRecorder` fires `dataavailable`; stop calls the real `finish` IPC and tears down every media track), and `RecordingCenter.test.tsx` (3 tests — source listing, starting a recording end-to-end through the real provider stack, and the empty state).

**New/changed files**: `src/shared/contracts/recording.ts` (new), `src/main/recording/RecordingService.ts` (new), `src/main/ipc/registerRecordingHandlers.ts` (new), `src/main/ipc/index.ts` (wiring), `src/shared/contracts/bridge.ts`/`src/preload/index.ts` (new `recording` bridge member), `src/renderer/src/services/ipc/recordingClient.ts` (new), `src/renderer/src/state/recordingContext.ts`/`useRecording.ts`/`recording.tsx`/`recordingMedia.ts` (new), `src/renderer/src/state/__tests__/recordingMedia.test.ts` (new, 6 tests), `src/renderer/src/state/__tests__/recording.test.tsx` (new, 2 tests), `src/renderer/src/features/recording/RecordingIndicatorOverlay.tsx`/`RecordingCenter.tsx` (new), `src/renderer/src/features/recording/__tests__/RecordingCenter.test.tsx` (new, 3 tests), `src/renderer/src/app/providers/AppProviders.tsx` (new provider in the tree), `src/renderer/src/app/routing/routes.tsx` (new `/recording` route), `src/renderer/src/features/system/SystemDashboard.tsx` (new link).

**Validation evidence (run 2026-06-29):**

```text
npm run test -> 223 files / 1073 tests passed
npm run typecheck -> passed
npm run lint -> passed
npm run build -> passed
```
...
**Why 9 of 10 categories are structurally advisory, not a policy choice**: `ApplicationLauncher` launches external applications as ordinary detached OS processes (`spawnDetached()`) — NeuroDeck does not run them inside any sandbox it controls. It therefore has no real mechanism to enforce file-roots/network/clipboard/microphone/camera/notifications/AI-context/extension-access/download-location restrictions on an already-launched external process. `ApplicationPolicyStore` persists real, user-visible allow/deny _declarations of intent_ for all nine — genuinely useful as a record of what the user expects, never dressed up as an enforced boundary.
...
**Nothing new had to be built at the data layer** — `ToolRegistry.list()`, `PermissionBroker.evaluate()`, and `AuditLog` were all already real and already consumed by Privacy and Permissions' own per-tool grant view. What didn't exist was a dedicated reference screen for _browsing the catalog itself_ — Privacy and Permissions is framed around managing grants (title + capability + revoke), not understanding what a tool does, its risk level, its reversibility, or its real usage history.
...
**A real side-effect worth recording**: this closes part of the reasoning chain that previously blocked skill packs (§14.4) — the spec requires skill packs to "use the same signing and permission model as extensions," and the prior blocker was that extension signing was presence-only, not cryptographically verified. Epic X15 (`ManifestSignature.ts`/`TrustedPublisherStore`) made that real. Skill packs are still not built, but the checklist now records the _actual_ remaining gap honestly: the bundle format itself (prompts + workflows + tools + learning content + controller shortcuts + documentation as one packaged/installable unit) doesn't exist, not the signing foundation it would need to sit on.
...
- **Validation**: the new content is parsed and checked _before_ it's ever written to disk (catching the codec's own bugs before they touch a real file), and the file is read back and re-parsed _after_ writing to confirm what's actually on disk matches what was intended — two real checks, not one assumed-successful write.
...
**Closed the one named remaining gap for Epic X6.** `ClipboardStore`/`SnippetStore` and their typed IPC (`clipboardClient.ts`/`snippetClient.ts`) were already real; no renderer screen mounted them. Two real gaps were found and fixed while building the screen, not just wiring:

1. **`ClipboardStore.add()` was never called by anything in the codebase.** The store, encryption, secret-shape rejection, and monitoring-enabled flag were all real, but nothing populated it — the feature was a real but permanently empty list. Electron/Chromium expose no clipboard-changed event, so `core/clipboard/ClipboardMonitor.ts` polls the real OS clipboard via `electron.clipboard.readText()` on a 1.5s interval and diffs against the last-seen value — the same honest polling technique `BackupScheduler` already established for its own missing OS hook. `ClipboardStore.add()` itself already enforces monitoring-enabled and secret-shape rejection, so the monitor never duplicates either check. Started/stopped alongside `backupScheduler` in `main/ipc/index.ts`.
2. **The monitoring toggle had no way to read its own current state.** `ClipboardStore.isMonitoringEnabled()` existed but was never exposed over IPC. Added `clipboard.getMonitoring` (channel, `ipcMain.handle`, preload bridge method, `getClipboardMonitoring()` renderer client) rather than shipping a toggle UI that could only write, never reflect reality.

**`/clipboard` (ND-X022, the spec's own combined "Clipboard and Snippet Center" screen grouping — not two separate screens)**: real search, pin/unpin, remove, clear-with-confirmation, copy-to-clipboard, and Share (reusing the existing real Universal Share Sheet via `useShareSheet().openShareSheet({ text, sourceLabel })` rather than a second dispatcher) for clipboard history; real create/edit/delete, `{{variable}}`-fill preview calling the real `snippet.render` IPC with honest missing-variable reporting, copy-rendered-text, and Share for snippets. Shell snippets show their real risk classification and reason inline. Reachable from System Dashboard and Command Palette, matching the exact pattern Screenshot Center/Voice Notes already established.

**New files**: `src/core/clipboard/ClipboardMonitor.ts` + `__tests__/ClipboardMonitor.test.ts` (5 tests), `src/renderer/src/features/clipboard/ClipboardSnippetCenter.tsx` + `__tests__/ClipboardSnippetCenter.test.tsx` (8 tests). **Changed**: `src/shared/contracts/ipcChannels.ts`/`bridge.ts` (`clipboard.getMonitoring`), `src/main/ipc/registerClipboardHandlers.ts`, `src/main/ipc/index.ts` (clipboard import + monitor start/stop), `src/preload/index.ts`, `src/renderer/src/services/ipc/clipboardClient.ts`, `src/renderer/src/app/routing/routes.tsx` (new `/clipboard` route), `src/renderer/src/features/system/SystemDashboard.tsx` and `src/renderer/src/features/command-palette/CommandPalette.tsx` (new links/entries).

**Validation evidence (run 2026-07-01):**

```text
npm run typecheck -> passed (node + web)
npm run lint -> 0 errors (10,873 pre-existing CRLF line-ending warnings across the working tree, unrelated to this change — git core.autocrlf on Windows)
npm run test -> 1209/1210 passed; 1 pre-existing flaky test (GuidedControllerTutorial.test.tsx, a waitFor timeout under parallel-suite load) confirmed to pass cleanly in isolation, not caused by this change
npm run build -> passed (ClipboardSnippetCenter bundled as its own lazy chunk)
```
