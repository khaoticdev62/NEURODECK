    ...
    <>
      ...
      <GridView>
        <ModelCard
          ...
          location="local"
          status={selected === "Llama 3.1 8B" ? "selected" : "available"}
          contextSize="8192"
          vram="6.2 GB"
          throughput="42"
          capabilities={["chat", "tools"]}
          onSelect={() => onSelect && onSelect("Llama 3.1 8B")}
        />
        <ModelCard
          name="Qwen 2.5 14B"
          provider="Alibaba · Ollama"
          location="local"
          status={selected === "Qwen 2.5 14B" ? "selected" : "available"}
          contextSize="32k"
          vram="9.8 GB"
          throughput="28"
          capabilities={["chat", "code"]}
          onSelect={() => onSelect && onSelect("Qwen 2.5 14B")}
        />
        <ModelCard
          name="Claude Sonnet"
          provider="Anthropic"
  ...
  return (
    <>
      ...
      <GridView>
        <AgentCard
          name="Recon"
          role="Read-only repo analyst"
          model="Llama 3.1 8B"
          trusted
          permissions={["filesystem", "web"]}
          runStatus="ok"
        />
        <AgentCard
          name="Operator"
          role="Shell + file edits"
          trusted={false}
        ...
        {checks.map((c) => (
          <Panel key={c.label} density="compact" emphasis="default">
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, color: "var(--nd-text-primary)" }}>
                {c.label}
              </span>
              <span
                style={{
