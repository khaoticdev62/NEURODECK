...
- **Acceptance Criteria**:
  - [x] Remove the duplicate root-level `.agent/` directory.
  - [x] Retain `.agents/` as the single source of truth for BMAD skills.
...
- **Acceptance Criteria**:
  - [x] Remove root-level empty `themes/` directory.
  - [x] Remove `src-tauri/plugins/` (since plugins are resolved dynamically from root `plugins/` or packaging).
...
- **Acceptance Criteria**:
  - [x] Move `build_flatpak.sh` to `scripts/shell/build_flatpak.sh`.
  - [x] Update `scripts/kfms/khaotic-init.sh` to remove `build_flatpak.sh` from the root preservation list.
  - [x] Update references to `build_flatpak.sh` in the release plan and documentation.
...
- **Acceptance Criteria**:
  - [x] Run `khaotic-init.sh status` and `validate` to confirm a clean workspace and health score.
  - [x] Verify both Tauri backend compiles and frontend builds successfully.
