...
async fn _run_orchestration(
    ...
    aborted!();
    let plan_raw = provider
        .generate_oneshot(&plan_prompt, 512)
        .await
    ...
    let parsed: serde_json::Value = serde_json::from_str(&plan_json).map_err(|e| {
        format!(
            "Plan JSON parse error: {} — raw: {}",
            e,
            &plan_json[..plan_json.len().min(200)]
        )
    })?;
    ...
    while !remaining.is_empty() && iterations < 20 {
        ...
        let ready: Vec<AgentTask> = remaining
            .iter()
        ...
        for task in ready.clone() {
            ...
            handles.push(tokio::spawn(async move {
                (
                    task.id.clone(),
                    prov.generate_oneshot(&exec_prompt, 800).await,
                )
            }));
        }
    ...
    let summary = provider
        .generate_oneshot(&synthesis_prompt, 600)
        .await
