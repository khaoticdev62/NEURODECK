...
pub fn load_schema(path: &str) -> Result<ControllerProfileSchema> {
    ...
    let schema: ControllerProfileSchema =
...
pub fn load_multilang_profile(path: &str) -> Result<multilang_schema::MultiLangProfileSchema> {
    ...
    let schema: multilang_schema::MultiLangProfileSchema =
        serde_json::from_str(&content).context("Failed to parse JSON")?;
    Ok(schema)
}

pub struct DeckCodeState(
    pub  std::sync::Mutex<(
        Option<ControllerProfileSchema>,
        Option<multilang_schema::MultiLangProfileSchema>,
    )>,
);
