...
pub async fn ollama_list_models(base_url: String) -> Result<Vec<OllamaModel>, String> {
    ...
    if !res.status().is_success() {
        return Err(format!("Ollama tags API returned error status: {}", res.status()));
    }
    
    let parsed = res.json::<OllamaTagsResponse>()
        .await
...
pub async fn ollama_pull_model(
    ...
    if !res.status().is_success() {
        return Err(format!("Ollama pull returned error status: {}", res.status()));
    }
                        ...
                        if !line.is_empty() {
                            if let Ok(pull_chunk) = serde_json::from_str::<OllamaPullChunk>(&line) {
                                let payload = PullProgressPayload {
                                    ...
                                    status: pull_chunk.status.unwrap_or_else(|| "downloading".to_string()),
...
pub async fn ollama_delete_model(base_url: String, model: String) -> Result<(), String> {
    ...
    if !res.status().is_success() {
        return Err(format!("Ollama delete returned error status: {}", res.status()));
    }
