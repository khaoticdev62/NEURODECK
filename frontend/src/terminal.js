...
function iconButtonMarkup(icon, label, extraClass = "", iconOnly = false) {
  const classes = [iconOnly ? "nd-icon-button nd-icon-only" : "nd-icon-button", extraClass]
    .filter(Boolean)
    .join(" ");
  const labelMarkup = iconOnly
    ? ""
    : `<span class="nd-button-label">${window.sanitizeHtml(label)}</span>`;
...
function profileActionButton(
  ...
  icon,
  label,
  { danger = false, iconOnly = false } = {}
) {
  const style = danger
    ? "padding:3px 8px;font-size:0.75rem;border-color:#ff3c5a;color:#ff6b81;"
  ...
  const term = new Terminal({
    ...
    theme: {
      background:
        ...
        "#000000",
      foreground:
        getComputedStyle(document.documentElement).getPropertyValue("--fg-color").trim() ||
        "#e2e8f0",
      cursor:
...
function createTerminalSession(shellPath) {
  if (state.terminalSessions.length >= MAX_TERMINAL_SESSIONS) {
    addNotification(
      "Tab Limit Reached",
      `Maximum of ${MAX_TERMINAL_SESSIONS} active terminal tabs allowed.`,
      "warning"
    );
    return;
  }
    ...
    if (s.id === id) {
      ...
      setTimeout(() => {
        try {
          ...
          if (dims) {
            invoke("pty_resize", { id: s.id, cols: dims.cols, rows: dims.rows }).catch(
              console.error
            );
          }
        } catch (e) {}
...
function renderTerminalTabs() {
  ...
  list.innerHTML = state.terminalSessions
    .map((s, idx) => {
  ...
  list.querySelectorAll(".terminal-tab-label").forEach((lbl) => {
    lbl.addEventListener("dblclick", (e) => {
      ...
      input.style.cssText =
        "width:80px;background:rgba(0,0,0,0.6);border:1px solid var(--accent-color);color:inherit;border-radius:3px;padding:1px 4px;font-size:0.75rem;font-family:var(--font-mono);outline:none;";
  ...
  if (session) {
    ...
    if (authType === "password" && passInput && passInput.value) {
      ...
      if (
        ...
        (lowerData.includes("password") && lowerData.trim().endsWith(":"))
      ) {
      ...
      if (typeof addNotification === "function") {
        ...
        if (typeof state !== "undefined" && state.activeProvider === "ollama") {
          setTimeout(() => {
            addNotification(
              "Provider Warning",
              "Vision requires the Gemini provider. Switch in Settings -> LLM.",
              "warning"
            );
          }, 500);
        }
...
function insertHistoryCommand(cmd) {
  ...
  const activeSession =
    typeof state.terminalSessions !== "undefined"
      ? state.terminalSessions.find((s) => s.id === state.activeTerminalSessionId)
      : null;
...
async function performHistorySearch(query) {
  ...
  try {
    ...
    if (body)
      body.innerHTML = `<div class="history-empty-state" style="color:var(--error-color);">Error: ${err}</div>`;
  ...
  if (hsInput) {
    hsInput.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Escape") {
        ...
        e.preventDefault();
        historySearchSelectedIdx = Math.min(
          historySearchSelectedIdx + 1,
...
function _termAutoOnData(data, buf) {
  ...
  if (data === "\x00" || (data === " " && autocompleteActive)) {
    /* Ctrl+Space / space-while-active placeholder */
  }
...
function initSshTerminal() {
  ...
  term.write(
    "\x1b[1;36mNEURODECK SSH Client\x1b[0m — Enter connection details and click Connect.\r\n"
  );
}
...
function getFtpProfiles() {
  try {
    return JSON.parse(localStorage.getItem("ftpProfiles") || "[]");
  } catch {
    return [];
  }
}
...
function initFtpProfileListeners() {
  document.getElementById("ftp-save-profile-btn")?.addEventListener("click", async () => {
    ...
    const name = await showPrompt("Profile name:", `${user}@${host}`, {
      title: "Save FTP Profile",
    });
    if (!name) return;
        ...
        .then(() => {
          ...
          if (typeof addNotification === "function")
            addNotification(
              ...
              `Downloaded file '${name}' to: ${localPath}`,
              "success"
            );
        })
        .catch((err) => {
...
function renderRemoteFiles(type, entries) {
  ...
  list
    .querySelectorAll(`.${prefix}-download-btn`)
    .forEach((btn) => _wireRemoteDownloadBtn(btn, isSftp));
}
...
async function deleteProfile(type, index) {
  const { getProfilesFn, saveProfilesFn, delCredentialFnName, updateFn1, updateFn2 } =
    getConnectionTypeMappers(type);
...
async function _sshLoadProfile(p) {
  ...
  if (p.auth_type === "password" && window.__TAURI_INTERNALS__) {
    try {
      ...
      console.error("Failed to check SSH credential:", err);
    }
  }
}
...
function _ftpLoadProfile(p) {
  const $ = (id) => document.getElementById(id);
  $("ftp-host-input").value = p.host || "";
  $("ftp-port-input").value = p.port || "21";
  $("ftp-user-input").value = p.user || "";
  $("ftp-pass-input").value = "";
  $("ftp-path-input").value = p.path || "/";
}
...
async function _sftpLoadProfile(p) {
  ...
  $("sftp-pass-input").value = "";
  $("sftp-path-input").value = p.path || "/";
  if (p.auth_type === "password" && window.__TAURI_INTERNALS__) {
    try {
      ...
      console.error("Failed to check SFTP credential:", err);
    }
  }
}
...
function renderConnectionProfiles(type) {
  ...
  list.querySelectorAll(`.${loadBtnClass}`).forEach((btn) => {
    btn.onclick = async () => {
      ...
      if (p) await loadHandler(p);
    };
  });
  list.querySelectorAll(`.${delBtnClass}`).forEach((btn) => {
    btn.onclick = async () => {
      await deleteProfile(type, parseInt(btn.getAttribute("data-index")));
    };
  });
}
...
function loadFtpDir(path) {
  ...
  if (cwdLabel)
    cwdLabel.innerHTML = `${createIcon("folder", { size: 14 })}<span>${window.sanitizeHtml(path)}</span>`;
  ...
  invoke("ftp_list_dir", { host, port, user, password, path })
    ...
    .catch((err) => {
      ...
      if (list)
        list.innerHTML = `<div class="ftp-empty-state" style="color:#ff6b6b;">Error: ${err}</div>`;
    });
}
...
function getSftpProfiles() {
  try {
    return JSON.parse(localStorage.getItem("sftpProfiles") || "[]");
  } catch {
    return [];
  }
}
...
function initFtpSftpDragDrop() {
  _wireDropzone(
    document.getElementById("ftp-dropzone"),
    document.getElementById("ftp-local-path-input"),
    document.getElementById("ftp-remote-dest-input"),
    "FTP"
  );
  _wireDropzone(
    document.getElementById("sftp-dropzone"),
    document.getElementById("sftp-local-path-input"),
    document.getElementById("sftp-remote-dest-input"),
    "SFTP"
  );
}
