...
const COLLAB_CLIENT_ID =
  window.crypto?.randomUUID?.() || `nd-${Date.now()}-${Math.random().toString(16).slice(2)}`;
...
function buildPreviewDoc(lang, code) {
  ...
  switch (lang) {
    case "html": {
      ...
      return stripped.toLowerCase().includes("<head>")
        ? stripped.replace(/<head>/i, `<head>${csp}`)
        : csp + stripped;
    }
    case "css":
...
try{${code}}catch(e){out.textContent+='\\n[Error] '+e.message}<\/script></body></html>`;
    case "markdown": {
      ...
      const html =
        parsed && typeof parsed.then === "function"
          ? ""
          : window.sanitizeHtml
            ? window.sanitizeHtml(parsed)
            : "";
...
function loadCanvasCode(lang, content, fileName = "") {
  ...
  const mappedLang = ["js", "javascript"].includes(normalizedLang)
    ? "javascript"
    : ["sh", "shell", "zsh", "bash"].includes(normalizedLang)
      ? "bash"
  ...
  if (monacoEditor) {
    ...
    if (model)
      window.monaco?.editor.setModelLanguage(
        model,
        getMonacoLang(window.neurodeckCanvas.currentLang)
      );
  }
  if (fileTitle)
    fileTitle.textContent =
      window.neurodeckCanvas.activePluginFile ||
      CANVAS_EXT_MAP[window.neurodeckCanvas.currentLang] ||
      "untitled";
  renderCanvasPreview();
}
    ...
    if (monacoEditor) {
      if (
        ...
        monacoEditor.executeEdits("ai-edit", [
          { range: selection, text: cleaned, forceMoveMarkers: true },
        ]);
      } else {
...
function ensureAiEditModal() {
  ...
  modal.addEventListener("click", (e) => {
    if (e.target === modal) closeAiEditModal();
  });
...
function createFallbackEditor(container, initialCode) {
  ...
  textarea.style.cssText =
    "width:100%;height:100%;background:#060a0e;color:#c9d1d9;font-family:monospace;font-size:13px;border:none;outline:none;padding:14px;box-sizing:border-box;resize:none";
  ...
  return {
    ...
    getModel() {
      return {
        getValueInRange(range) {
          ...
          const end = Math.max(
            start,
            Math.min(range?.endOffset ?? textarea.value.length, textarea.value.length)
          );
    ...
    getSelection() {
      return { startOffset: textarea.selectionStart ?? 0, endOffset: textarea.selectionEnd ?? 0 };
    },
    executeEdits(_source, edits) {
      ...
      const end = Math.max(
        start,
        Math.min(edit.range?.endOffset ?? textarea.value.length, textarea.value.length)
      );
...
function _cvStopExecListeners(cv) {
  if (cv.execLineUnlisten) {
    cv.execLineUnlisten();
    cv.execLineUnlisten = null;
  }
  if (cv.execDoneUnlisten) {
    cv.execDoneUnlisten();
    cv.execDoneUnlisten = null;
  }
}
...
async function _cvWireExecListeners(cv, outputPre) {
  ...
  cv.execDoneUnlisten = await listen("canvas_exec_done", (event) => {
    ...
    window.addNotification?.(
      "Canvas Exec",
      `Finished in ${duration} (exit ${exitCode})`,
...
function _cvWireDivider(cv) {
  ...
  cv.divider.addEventListener("mousedown", (e) => {
    isDragging = true;
    e.preventDefault();
...
async function _ccRefreshPeerList(cc) {
  ...
  try {
    ...
    if (peers.length === 0) {
      ...
      cc.peerListEl.innerHTML = peers
        .map((p) => {
          const name = p.name || p.hostname || "Unknown",
            addr = p.addr || `${p.ip}:${p.port}`;
      ...
      cc.peerListEl.querySelectorAll(".collab-peer-item").forEach((item) => {
        item.addEventListener("click", () => {
          if (cc.addrInput) cc.addrInput.value = item.dataset.addr;
        });
      });
    }
  } catch (e) {
...
function _ccStopPeerDiscovery(cc) {
  if (cc.peerDiscoveryInterval) {
    clearInterval(cc.peerDiscoveryInterval);
    cc.peerDiscoveryInterval = null;
  }
}
...
function _ccShowTab(tab, cc) {
  ...
  if (cc.hostTabBtn) {
    cc.hostTabBtn.style.background = isHost ? "rgba(0,229,255,0.1)" : "";
    cc.hostTabBtn.style.borderColor = isHost ? "var(--accent-color)" : "";
  }
  if (cc.joinTabBtn) {
    cc.joinTabBtn.style.background = isHost ? "" : "rgba(0,229,255,0.1)";
    cc.joinTabBtn.style.borderColor = isHost ? "" : "var(--accent-color)";
  }
...
function _ccUpdatePresenceList(cc) {
  ...
  cc.presenceList.innerHTML = Array.from(collabPresence.entries())
    .map(
      ([id, peer]) =>
...
async function _ccBroadcast(payload, cc) {
  const enriched = {
    ...
    sender: COLLAB_CLIENT_ID,
    workspace:
      (cc.workspaceNameInput?.value || "NEURODECK Workspace").trim() || "NEURODECK Workspace",
...
function _ccSetPeerConnected(peerInfo = "", cc) {
  ...
  if (cc.collabBtn) {
    cc.collabBtn.style.background = "rgba(0,255,136,0.15)";
    cc.collabBtn.style.borderColor = "var(--response-color)";
  }
  if (cc.statusBar) cc.statusBar.style.display = "flex";
  if (cc.statusText) {
    cc.statusText.innerText = peerInfo
      ? `Collab Active: Connected to peer (${peerInfo})`
      : cc.addrInput?.value
        ? `Collab Active: Connected to ${cc.addrInput.value}`
        : "Collab Active: Syncing edits live";
  }
...
function _ccSetDisconnected(cc) {
  ...
  if (cc.hostWaiting) cc.hostWaiting.style.display = "none";
  if (cc.collabBtn) {
    cc.collabBtn.style.background = "";
    cc.collabBtn.style.borderColor = "";
  }
  if (cc.statusLine) cc.statusLine.innerHTML = "";
...
function _ccUpdateInvitePayload(address, cc) {
  ...
  cc.invitePayload.value = JSON.stringify(
    {
...
function _ccHandleCanvasSyncEvent(data, cc) {
  ...
  if (
    (data.type === "sync" || data.type === "y_update" || data.type === "sync_full") &&
    data.code !== undefined
  ) {
    const sender = data.sender || "unknown";
    if (!cc.approvedSyncPeers.has(sender)) {
      cc.pendingSyncs.set(sender, data);
      _ccAppendSyncApproval(sender, data.name, cc);
      return;
    }
    ...
    _ccAppendApproval(
      `${data.name || "Peer"} requested agent approval: ${data.action || "shared run"}`,
      "pending",
      {
        remote: true,
    ...
    addNotification(
      "Shared Agent Approval",
      data.action || "A collaborator requested approval.",
      "warning"
    );
  } else if (data.type === "agent_approval_response") {
    _ccAppendApproval(
      `${data.name || "Peer"} ${data.approved ? "approved" : "denied"}: ${data.action || "shared run"}`,
...
function _ccWireCollabEvents(cc) {
  listen("canvas_collab_event", (event) => {
    ...
    if (msg.startsWith("peer_connected")) {
      ...
      setCanvasStatusLine(cc.statusLine, msg, "error");
    }
  }).catch(() => {});
...
function _ccWireModalOpen(cc) {
  ...
  if (cc.closeX)
    cc.closeX.addEventListener("click", () => {
      cc.collabModal.classList.remove("active");
      cc.collabModal.setAttribute("aria-hidden", "true");
      cc.collabFocusTrap?.deactivate();
      _ccStopPeerDiscovery(cc);
    });
  cc.collabModal.addEventListener("click", (e) => {
    if (e.target === cc.collabModal) {
      cc.collabModal.classList.remove("active");
      cc.collabModal.setAttribute("aria-hidden", "true");
      cc.collabFocusTrap?.deactivate();
      _ccStopPeerDiscovery(cc);
  ...
  if (cc.hostStartBtn) {
    cc.hostStartBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        setCanvasStatusLine(cc.statusLine);
        _ccRefreshCollabStatus(cc);
      } catch (err) {
        setCanvasStatusLine(cc.statusLine, `Error: ${String(err)}`, "error");
        cc.hostStartBtn.disabled = false;
      }
    });
  }
  if (cc.joinStartBtn) {
    cc.joinStartBtn.addEventListener("click", async () => {
      ...
      if (!addr) {
        setCanvasStatusLine(cc.statusLine, "Enter the host address first.", "warning");
        return;
      }
      ...
      try {
        ...
        _ccSetPeerConnected(addr, cc);
        _ccUpdateInvitePayload(addr, cc);
      } catch (err) {
        setCanvasStatusLine(cc.statusLine, `Error: ${String(err)}`, "error");
        cc.joinStartBtn.disabled = false;
      }
    });
  }
  if (cc.stopBtn) {
...
function _ccWireChat(cc) {
  if (cc.chatSendBtn && cc.chatInput) {
    ...
    cc.chatInput.addEventListener("keydown", (event) => {
      if (event.key === "Enter") cc.chatSendBtn.click();
    });
  }
  if (cc.approvalBtn) {
...
function initCanvasView() {
  ...
  if (select) {
    select.addEventListener("change", () => {
      ...
      if (fileTitle)
        fileTitle.textContent =
          window.neurodeckCanvas.activePluginFile || CANVAS_EXT_MAP[select.value] || "untitled";
      if (monacoEditor) {
        const model = monacoEditor.getModel();
        if (model && window.monaco?.editor)
          window.monaco.editor.setModelLanguage(model, getMonacoLang(select.value));
      }
      renderCanvasPreview();
    });
  }
  ...
  if (!cancelBtn && runBtn) {
    ...
    cancelBtn.style.cssText =
      "display:none;border-color:var(--error-color);color:var(--error-color)";
  ...
  if (clearBtn)
    clearBtn.onclick = async () => {
      const confirmed = await showConfirm("Clear the editor?", {
        confirmText: "Clear",
        cancelText: "Keep",
      });
      if (confirmed) {
  ...
  if (copyBtn)
    copyBtn.onclick = () => {
      ...
      navigator.clipboard.writeText(code).then(() => {
        ...
        setTimeout(
          () => applyButtonIcon("#canvas-copy-btn", { icon: "copy", label: "Copy" }),
          1500
        );
      });
    };
...
export { buildPreviewDoc, renderCanvasPreview, loadCanvasCode, initCanvasView, initCanvasCollab };
