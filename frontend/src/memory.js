...
function _memBuildRecordCard(record, ctx) {
    ...
    const sourceRow = sourcePath ? `<div class="memory-record-source">${_memEscHtml(sourcePath)}</div>` : "";
    const chunkBc   = (chunkIdx !== undefined && chunkTotal !== undefined)
    ...
    card.querySelector(".mem-pin-btn").onclick = async function() {
        ...
        try {
            ...
            if (rec) { if (!wasPinned) rec.metadata.pinned = "true"; else delete rec.metadata.pinned; }
            _memRenderList(ctx);
    ...
    card.querySelector(".mem-del-btn").onclick = async function() {
        const id = this.dataset.id;
        const confirmed = await showConfirm("Delete this memory record?", { confirmText:"Delete", cancelText:"Keep" });
        if (!confirmed) return;
        try {
...
function _memRenderList(ctx) {
    ...
    if      (ctx.activeFilter === "pinned")              filtered = ctx.allRecords.filter(r => r.metadata.pinned === "true");
    else if (ctx.activeFilter === "user")                filtered = ctx.allRecords.filter(r => r.metadata.role === "user");
    else if (ctx.activeFilter === "ai")                  filtered = ctx.allRecords.filter(r => r.metadata.role === "ai");
    else if (ctx.activeFilter === "fact")                filtered = ctx.allRecords.filter(r => r.metadata.role === "fact");
    ...
    if (totalCount)    totalCount.textContent   = `${ctx.allRecords.length} record${ctx.allRecords.length !== 1 ? "s" : ""}`;
    ...
    if (filtered.length === 0) {
        if (emptyState) {
            ...
            emptyState.querySelector("p").textContent = query || ctx.activeFilter !== "all"
                ? "No records match this filter." : "No memory records yet.";
        }
        return;
    }
...
function _memBuildBackupRow(b, ctx) {
    ...
    row.querySelector(".memory-btn-restore").onclick = async function() {
        ...
        const confirmed = await showConfirm(`Restore from backup "${name}"?\nThis will REPLACE all current memory records.`, { confirmText:"Restore", cancelText:"Cancel" });
        if (!confirmed) return;
...
async function _memRenderBackupList(ctx) {
    ...
    try {
        ...
        if (backups.length === 0) { backupList.innerHTML = '<div class="memory-backup-empty">No backups yet. Click 💾 Backup to create one.</div>'; return; }
...
function _memInitDataButtons(ctx) {
    ...
    if (exportBtn) {
        exportBtn.onclick = async function() {
            ...
            try { const path = await invoke("memory_export"); window.addNotification?.("Memory Exported", `Saved to: ${path}`, "success"); }
            catch (e) { window.addNotification?.("Export Failed", String(e), "error"); }
    ...
    if (importFile) {
        importFile.onchange = async function() {
            ...
            if (!file) return;
            const merge = await showConfirm(`Import "${file.name}"?\n\nClick OK to MERGE.\nClick Cancel to REPLACE all current memory.`);
            if (importLabel) importLabel.textContent = "Importing…";
            try {
                const count = await invoke("memory_import_data", { data: await file.text(), merge });
                window.addNotification?.("Memory Imported", `${count} records ${merge ? "merged" : "replaced"}.`, "success");
                await _memLoad(ctx);
            } catch (e) { window.addNotification?.("Import Failed", String(e), "error"); }
            finally { if (importLabel) importLabel.textContent = "⬇ Import"; importFile.value = ""; }
        };
    }
    ...
    if (backupBtn) {
        backupBtn.onclick = async function() {
            ...
            try { const path = await invoke("memory_backup_auto"); window.addNotification?.("Backup Created", `Saved to: ${path}`, "success"); }
            catch (e) { window.addNotification?.("Backup Failed", String(e), "error"); }
...
export function initMemoryView() {
    ...
    if (ctx.searchInput) {
        let debounce = null;
        ctx.searchInput.addEventListener("input", () => { clearTimeout(debounce); debounce = setTimeout(() => _memRenderList(ctx), 200); });
    }
    ...
    if (ctx.factInput)   ctx.factInput.addEventListener("keydown", e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); _memSaveFact(ctx); } });
