      ...
      {visible.map((memory) => (
        <div
          ...
          className={`rounded-xl border bg-nd-surface/30 transition hover:border-nd-accent-primary/20 ${
            memory.pinned
              ? "border-nd-accent-primary/20 bg-nd-accent-primary/[0.04]"
              : "border-nd-text-muted/15"
          }`}
        >
          <div
            className={`flex items-start justify-between gap-2 ${compact ? "px-3 py-2" : "px-3 pt-2.5"}`}
          >
            <p className="min-w-0 flex-1 text-xs font-medium leading-5 text-nd-text-primary/90 truncate">
              {memory.title}
            </p>
                ...
                <button
                  ...
                  className={`rounded p-0.5 transition ${
                    memory.pinned
                      ...
                      : "text-nd-text-muted/40 hover:text-nd-accent-primary"
                  }`}
                >
          ...
          {!compact && (
            <p className="px-3 pb-2.5 text-[11px] leading-4 text-nd-text-muted line-clamp-2">
              {memory.body}
            </p>
          )}
        </div>
