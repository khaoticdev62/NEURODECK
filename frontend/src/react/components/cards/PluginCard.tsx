...
export function PluginCard({ plugin, onToggle }: PluginCardProps) {
  return (
    <article className="rounded-3xl border border-nd-text-muted/15 bg-nd-surface/40 p-4 transition hover:border-nd-accent-primary/25">
      ...
      <h3 className="mt-4 font-semibold text-nd-text-primary">{plugin.name}</h3>
      ...
      <div className="mt-4 space-y-2">
        {plugin.permissions.map((perm) => (
          <div
            key={perm}
            className="flex items-center gap-2 rounded-xl border border-nd-text-muted/15 bg-nd-surface/30 px-3 py-2 text-xs text-nd-text-muted"
          >
            <ShieldAlert className="h-3.5 w-3.5 text-nd-accent-warning" aria-hidden="true" /> {perm}
          </div>
        ))}
      </div>
      ...
      <button
        ...
        className="mt-4 w-full rounded-xl border border-nd-accent-primary/25 bg-nd-accent-primary/10 px-3 py-2 text-sm font-semibold text-nd-accent-primary transition hover:bg-nd-accent-primary/15"
      >
