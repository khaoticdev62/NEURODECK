...
export function ModelCard({ model, selected, policyAllowed, policyReason, agentPreferred, onMarkReady, onMarkIndexed, onDisable, onSelect }: ModelCardProps) {
  return (
    <article
      className={`rounded-3xl border p-4 transition hover:border-nd-accent/30 ${
        selected ? 'border-nd-accent/40 bg-nd-accent/[0.07]' : policyAllowed === false ? 'border-nd-danger/25 bg-nd-danger/[0.04]' : 'border-nd-text-muted/15 bg-nd-surface/40'
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex gap-3">
          <div className={`flex h-12 w-12 items-center justify-center rounded-2xl border text-nd-accent ${policyAllowed === false ? 'border-nd-danger/25 bg-nd-danger/10 text-nd-danger' : 'border-nd-accent/25 bg-nd-accent/10'}`}>
      ...
      {policyAllowed === false && policyReason && (
        <p className="mt-3 rounded-xl border border-nd-danger/20 bg-nd-danger/10 px-3 py-2 text-xs text-nd-danger/90">{policyReason}</p>
      )}
