...
export function ModelCard({ model, selected, onMarkReady, onMarkIndexed, onDisable, onSelect }: ModelCardProps) {
  return (
    <article
      className={`rounded-3xl border p-4 transition hover:border-nd-accent/30 ${
        selected ? 'border-nd-accent/40 bg-nd-accent/[0.07]' : 'border-nd-text-muted/15 bg-nd-surface/40'
      }`}
    >
