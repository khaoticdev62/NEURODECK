...
export function Badge({
  ...
  return (
    ...
    >
      {dot && (
        <span
          className={`mr-1.5 h-1.5 w-1.5 rounded-full ${tone === "accent" ? "bg-nd-accent" : tone === "success" ? "bg-nd-success" : tone === "warning" ? "bg-nd-warning" : tone === "danger" ? "bg-nd-danger" : "bg-nd-text-muted"}`}
        />
      )}
      {children}
    </span>
