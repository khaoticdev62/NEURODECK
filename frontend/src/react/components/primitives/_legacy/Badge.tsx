...
export function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border ${sizeCls} ${variantCls} ${className}`}
    >
      {dot && (
        <span
          className={`mr-1.5 h-1.5 w-1.5 rounded-full ${tone === "accent" ? "bg-nd-accent-primary" : tone === "success" ? "bg-nd-accent-success" : tone === "warning" ? "bg-nd-accent-warning" : tone === "danger" ? "bg-nd-accent-error" : "bg-nd-text-muted"}`}
        />
      )}
      {children}
    </span>
