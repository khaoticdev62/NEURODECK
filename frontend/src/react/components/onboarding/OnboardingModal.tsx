...
import { StepEnvironment, type InstallerItemState, type InstallerProgressMap } from './steps/StepEnvironment';
...
function wizardReducer(state: WizardNav, action: WizardNavAction): WizardNav {
  switch (action.type) {
    ...
    case 'complete-and-advance': {
      ...
      const completed = [...state.completedSteps.filter((s) => s !== state.currentStep), state.currentStep];
...
function SetupOnboardingModal({
  ...
  const runDiagnostics = useCallback(async () => {
    ...
    try {
      ...
      if (!res.pty_ok) {
        errors.push({ code: 'PTY_FAIL', message: 'PTY Terminal Subsystem failed to spawn.', fix: 'Ensure shell terminal binary access permissions are configured correctly.' });
      }
      if (!res.network_ok) {
        if (res.network_details.includes('restricted')) {
          warnings.push({ code: 'API_RESTRICTED', message: 'Internet active, but Gemini Cloud API endpoints are restricted.', fix: 'Check proxy settings, firewall rules, or DNS configuration.' });
        } else {
          errors.push({ code: 'NET_DOWN', message: 'Network connection unreachable.', fix: 'Check local router connections, wireless switches, or network cables.' });
        }
      }
      if (!res.keychain_ok) warnings.push({ code: 'KEYCHAIN_FAIL', message: 'System keychain / secure credential storage is unavailable.', fix: 'Ollama works, but Gemini API keys will fall back to local storage.' });
      if (!res.audio_ok) warnings.push({ code: 'AUDIO_UNAVAILABLE', message: 'No active audio input device detected.', fix: 'Voice command STT capabilities require an active microphone input.' });
      if (!res.ssh_ok) warnings.push({ code: 'SSH_MISSING', message: 'SSH client binary was not found in system PATH.', fix: 'Remote SSH connections and secure loopback tunnel services require OpenSSH.' });
      if (!res.tts_ok) warnings.push({ code: 'TTS_MISSING', message: 'No supported Speech Synthesis TTS engine discovered.', fix: 'Voice responses require espeak-ng (Linux) or Windows SAPI.' });
      if (!status.openvpn) warnings.push({ code: 'OPENVPN_MISSING', message: 'OpenVPN client binary was not found in system PATH.', fix: 'VPN connections via OpenVPN require the OpenVPN client binary.' });
      if (!status.wireguard) warnings.push({ code: 'WIREGUARD_MISSING', message: 'WireGuard client binary was not found in system PATH.', fix: 'VPN connections via WireGuard require the WireGuard client binary.' });

      setDiagnosticsErrors(errors);
      setDiagnosticsWarnings(warnings);
    } catch (e) {
      setDiagnosticsErrors([{ code: 'DIAG_CRASH', message: `Diagnostics crashed: ${String(e)}`, fix: 'Restart bridge server sidecar and try again.' }]);
    } finally {
  ...
  const testModelConnection = async () => {
    ...
    try {
      ...
      if (match?.state === 'connected') {
        setTestResult({ success: true, message: `Successfully connected to ${providerType === 'lmstudio' ? 'LM Studio' : providerType.toUpperCase()}. Found models: ${match.models.join(', ') || 'none'}` });
      } else {
        setTestResult({ success: false, message: `Connection timed out or returned offline: ${match?.error || match?.state || 'Unknown response'}` });
      }
    } catch (e) {
      setTestResult({ success: false, message: `Connection failed: ${String(e)}. Double check endpoint URL and server status.` });
    } finally {
  ...
  const loadPluginsCheck = async () => {
    ...
    try {
      ...
      setPluginStats({ installed: list.length, active: list.filter((p) => p.enabled).length, disabled: list.filter((p) => !p.enabled).length, errors: errCount });
    } catch (_) {
  ...
  const savePreferences = async () => {
    await updateSettings({ activeThemeId: themeId, fontScale, compactMode, accessibilityProfile: reducedMotion ? 'reduced_motion' : 'default' });
  ...
  const dismiss = async (status: 'completed' | 'skipped') => {
    const statePayload = {
      status, currentStep, completedSteps, skippedSteps: [],
      warnings: diagnosticsWarnings, errors: diagnosticsErrors,
  ...
  const handleStepClick = async (stepId: OnboardingStep) => {
    ...
    if (targetIdx < currentIdx) { wizardDispatch({ type: 'go-to', step: stepId }); return; }
    if (currentStep === 'environment' && diagnosticsErrors.length > 0 && targetIdx > currentIdx) return;
    if (currentStep === 'models') await saveModels();
    else if (currentStep === 'preferences') await savePreferences();
  ...
  useEffect(() => { void runPrecheck(); }, [runPrecheck]);

  useEffect(() => {
    if (!neurodeckApi.dependency?.onProgress) return;
    type ProgEvent = { id: string; state: string; percent?: number; speed?: number; error?: string };
    const unsub = neurodeckApi.dependency.onProgress((data: ProgEvent) => {
      setInstallerProgress((prev) => ({ ...prev, [data.id]: { state: data.state, percent: data.percent, speed: data.speed, error: data.error } as InstallerItemState }));
  ...
  useEffect(() => {
    if (providerType === 'ollama') { setEndpointUrl('http://127.0.0.1:11434'); setModelId('llama3'); }
    else if (providerType === 'lmstudio') { setEndpointUrl('http://127.0.0.1:1234/v1'); setModelId('meta-llama-3-8b-instruct'); }
    else if (providerType === 'openai_compat') { setEndpointUrl('https://api.openai.com/v1'); setModelId('gpt-4o-mini'); }
    setTestResult(null);
  }, [providerType]);
  ...
  useEffect(() => {
    const handleKeydown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        if (precheckPassed && (currentStep === 'welcome' || currentStep === 'finish')) void dismiss('skipped');
        return;
      }
      if (e.key !== 'Tab') return;
      const focusable = Array.from(wizardRef.current.querySelectorAll<HTMLElement>(FOCUSABLE_ELEMENTS))
  ...
  useControllerAction('back', () => {
    if (currentStep === 'welcome') { if (precheckPassed) void dismiss('skipped'); return true; }
    handleBack(); return true;
  }, true);
  useControllerAction('cancel', () => {
    if (currentStep === 'welcome' || currentStep === 'finish') { if (precheckPassed) void dismiss('skipped'); return true; }
    handleBack(); return true;
  }, true);
  useControllerAction('forward', () => {
    if (isNextDisabled()) return true;
    void handleNext(); return true;
  }, true);
  useControllerAction('openSearch', () => {
    const target = wizardRef.current?.querySelector<HTMLElement>('input:not(:disabled), select:not(:disabled), textarea:not(:disabled)');
  ...
  const handleInstall = async (id: string) => {
    ...
    try {
      ...
      setInstallerProgress((prev) => ({ ...prev, [id]: { state: 'failed', error: (e as Error).message || String(e) } }));
    }
  };
