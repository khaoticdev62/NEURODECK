  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-[var(--nd-text-muted)]">Link your local or remote LLM endpoint sources.</p>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <Panel title="Provider Advice" variant="surface" className="flex flex-col justify-between">
          <div className="space-y-2 text-xs leading-relaxed text-[var(--nd-text-muted)]">
            {providerType === 'ollama' && (
              <p>Ollama runs models locally on your system. Ensure Ollama is running in the background and you have pulled your target model (e.g. <code className="rounded bg-[var(--nd-surface-tertiary)] px-1 py-0.5 text-[var(--nd-text-code)]">ollama pull llama3</code>).</p>
            )}
            {providerType === 'lmstudio' && (
              <p>LM Studio lets you host local models with an OpenAI-compatible server. Start LM Studio and run the local server on port 1234.</p>
            )}
            {providerType === 'openai_compat' && (
              <p>Use any custom API provider following the OpenAI spec (e.g., OpenRouter, TogetherAI, Groq). An API key is required.</p>
            )}
            {providerType === 'skip' && (
              <p className="text-[var(--nd-accent-warning)]">Selecting offline planning skips model setup. You can still use code terminals and files, but LLM chat requests will return offline fallback responses.</p>
            )}
          </div>
