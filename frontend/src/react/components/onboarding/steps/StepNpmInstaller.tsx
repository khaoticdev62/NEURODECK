...
import { AlertTriangle, Check, Download, PackagePlus, RefreshCw, TerminalSquare } from 'lucide-react';
...
export function StepNpmInstaller() {
  ...
  const canInstall = nodeAvailable && packageNameValid && trimmedName.length > 0 && !loading && !installing;
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs leading-5 text-[var(--nd-text-muted)]">
            Install one npm tool into NEURODECK&apos;s isolated package prefix before choosing the curated tool bundle.
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      {!nodeAvailable && status && (
        <div className="flex items-start gap-3 rounded-[var(--nd-radius-md)] border border-[rgba(var(--nd-yellow-rgb),0.22)] bg-[rgba(var(--nd-yellow-rgb),0.08)] p-4 text-xs leading-5 text-[var(--nd-accent-warning)]">
          ...
          <div>
            <p className="font-semibold">Node.js and npm are required before package installation.</p>
            <p className="mt-1 text-[var(--nd-text-muted)]">
              Install the current Node.js LTS from nodejs.org or your OS package manager, then return here and refresh.
            </p>
          </div>
        </div>
      )}
          ...
          {progress && (
            <div className="rounded-[var(--nd-radius-md)] border border-[rgba(var(--nd-cyan-rgb),0.18)] bg-[rgba(var(--nd-cyan-rgb),0.05)] p-3">
              <div className="flex items-center justify-between gap-3 text-xs">
                <span className="truncate font-medium text-[var(--nd-text-primary)]">{progress.name}</span>
                <StatusChip
                  tone={progress.state === 'failed' ? 'error' : progress.state === 'completed' ? 'success' : 'info'}
                  size="sm"
                >
                  {progress.state}
                </StatusChip>
      ...
      <Panel variant="surface" className="p-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-[var(--nd-text-primary)]">Installed onboarding tools</h3>
        </div>
        {packages.length === 0 ? (
          ...
          <div className="mt-3 max-h-28 space-y-2 overflow-y-auto pr-1 scrollbar-thin">
            {packages.slice(0, 8).map((pkg) => (
              ...
              >
                <span className="min-w-0 truncate text-xs font-medium text-[var(--nd-text-primary)]">{pkg.name}</span>
