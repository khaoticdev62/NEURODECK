...
function injectDsTokens(root: HTMLElement, tokens: ThemeTokenSet) {
  ...
  root.style.setProperty("--nd-elevation-card", tokens.shadow?.panel ?? "0 4px 16px rgba(0,0,0,0.6)");
  root.style.setProperty("--nd-elevation-overlay", tokens.shadow?.modal ?? "0 8px 32px rgba(0,0,0,0.8)");
  root.style.setProperty(
    "--nd-elevation-focus",
    `0 0 0 2px ${c.border.focus}, 0 0 24px rgba(${hexToRgb(c.accent.primary)}, 0.18)`
  );
  root.style.setProperty("--nd-elevation-glow", tokens.shadow?.glow ?? `0 0 16px rgba(${hexToRgb(c.accent.primary)}, 0.18)`);
