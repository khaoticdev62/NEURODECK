...
const LSP_DOT: Record<
  LspStatus,
  { tone: "success" | "warning" | "neutral" | "danger"; title: string }
> = {
...
export function LanguageModeBadge({
  ...
  return (
    <div
      ...
      role="status"
      aria-label={`Language: ${displayName ?? languageId}, LSP: ${lspStatus}, Mode: ${ideMode}`}
    >
      <Badge tone={dot.tone} variant="outline" dot size="sm">
        {displayName ?? languageId}
      </Badge>
      <Badge tone="accent" variant="fill" size="sm">
        {modeLabel}
      </Badge>
    </div>
  );
