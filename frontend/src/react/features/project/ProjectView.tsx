...
export function ProjectView({ state, actions }: { state: NeuroDeckState; actions: NeuroDeckAppActions }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard icon={FolderOpen} label="Project" value={project.name} hint={project.packageManager} />
        <MetricCard icon={PackageCheck} label="Files" value={project.stats.fileCount} hint={`${project.stats.directoryCount} folders scanned`} />
        <MetricCard icon={ClipboardCheck} label="Docs" value={`${docsReady}/4`} hint="README, env, changelog, gitignore" />
        <MetricCard icon={AlertTriangle} label="Risks" value={project.risks.length} hint="release blockers found" />
      </section>
        ...
        <Panel eyebrow="Project Scanner" title={project.name} className="min-h-0 overflow-hidden">
          <div className="h-full overflow-y-auto p-4 scrollbar-thin">
            ...
            <section className="mt-4 grid gap-4 lg:grid-cols-2">
              ...
              <InfoCard title="Scripts" items={Object.entries(project.scripts).map(([name, value]) => `${name}: ${value}`)} empty="No package scripts detected." />
              <Checklist title="Documentation Signals" items={project.docs} />
              <Checklist title="Testing Signals" items={project.testSignals} readyCount={testReady} />
            </section>

            <section className="mt-4 grid gap-4 lg:grid-cols-2">
              <InfoCard title="Risks" items={project.risks} empty="No obvious risks from this lightweight scan." tone={project.risks.length ? 'danger' : 'success'} />
              <InfoCard title="Recommended Next Actions" items={project.recommendations} tone="success" />
            </section>
          </div>
        </Panel>
