...
import { GitBranch, RefreshCw, GitCommit, GitMerge, FilePlus, FileMinus, CircleDot, AlertTriangle, ArrowUpFromLine } from 'lucide-react';
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 gap-3">
        ...
        <div className="flex w-64 shrink-0 flex-col gap-3 overflow-auto">
          <Panel className="p-3">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-nd-text-muted">Staged</h3>
              <Button
                variant="ghost"
                size="xs"
            ...
            {staged.map((f) => (
              <FileItem key={f.path} file={f} icon={FilePlus} color="text-nd-accent-success" statusLabel="A" onDiff={showDiff} />
            ))}
            {!staged.length && (
          ...
          <Panel className="p-3">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-nd-text-muted">Unstaged</h3>
              <Button
                variant="ghost"
                size="xs"
            ...
            {unstaged.map((f) => (
              <FileItem key={f.path} file={f} icon={CircleDot} color="text-nd-accent-warning" statusLabel="M" onDiff={showDiff} />
            ))}
            {!unstaged.length && (
          ...
          <Panel className="p-3">
            <h3 className="mb-1 text-xs font-semibold uppercase tracking-wider text-nd-text-muted">Untracked</h3>
            {untracked.map((f) => (
              <FileItem key={f.path} file={f} icon={FileMinus} color="text-nd-text-muted" statusLabel="?" onDiff={showDiff} />
            ))}
            {!untracked.length && (
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-3">
          <Panel className="p-3">
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-nd-text-muted">Commit</h3>
          ...
          {diff && (
            <Panel className="min-h-0 flex-1 overflow-auto p-3">
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-nd-text-muted">Diff</h3>
              <pre className="font-mono text-xs text-nd-text-primary/80">{diff}</pre>
            </Panel>
          )}

          <Panel className="min-h-0 flex-1 overflow-auto p-3">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-nd-text-muted">Recent Commits</h3>
            <ul className="mt-2 space-y-2" aria-label="Commit history">
              {commits.map((c) => (
                <li key={c.hash} className="rounded-lg border border-nd-border-subtle bg-nd-surface-secondary/30 p-2">
