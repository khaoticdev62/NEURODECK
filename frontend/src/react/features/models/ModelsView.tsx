...
export function ModelsView({ state, dispatch, actions }: { state: NeuroDeckState; dispatch: Dispatch<NeuroDeckAction>; actions: NeuroDeckAppActions }) {
  ...
  const allowedById = useMemo(() => Object.fromEntries(allowedModels.map((m) => [m.modelId, m])), [allowedModels]);
      ...
      <Panel eyebrow="Runtime Probe" title="Native Detection" className="flex flex-col min-h-0 overflow-hidden">
        <div className="flex-1 min-h-0 space-y-3 overflow-y-auto p-4 scrollbar-thin">
          ...
          <div className="rounded-2xl border border-nd-text-muted/15 bg-nd-surface/40 p-4">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-nd-text-muted/70">Active agent</p>
            ...
            {state.agentPolicies.find((p) => p.agentId === state.activeAgentId) && (
              <p className="mt-2 text-xs text-nd-text-muted">Models are filtered by this agent&apos;s policy.</p>
            )}
          </div>
          ...
          {state.modelDetection ? (
            <>
              ...
              {Object.keys(scoresMap).length > 0 && (
                <div className="space-y-2">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-nd-text-muted/70">Compatibility</p>
                  {state.models.map((model) => {
                    const score = scoresMap[model.id];
                    if (!score) return null;
                    return (
                      <div key={`score-${model.id}`} className="flex items-center justify-between rounded-xl border border-nd-text-muted/15 bg-nd-surface/40 px-3 py-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge tone={score.score >= 70 ? 'success' : score.score >= 40 ? 'warning' : 'danger'}>{score.tier}</Badge>
                        </div>
                      </div>
                    );
                  })}
            ...
            <p className="rounded-2xl border border-nd-text-muted/15 bg-nd-surface/40 p-4 text-sm leading-6 text-nd-text-muted">
              Detection runs through the NEURODECK bridge server and checks configured local model runtimes. No cloud calls.
            </p>
          )}
        </div>
      </Panel>
