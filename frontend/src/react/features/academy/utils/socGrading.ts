...
export function gradeAlertAnalysis(
  ...
  const escalationScore = (() => {
    ...
    const hits = alert.escalationKeywords.filter((kw) => normalized.includes(kw.toLowerCase()));
...
function buildFeedback(
  ...
  if (score >= 90)
    return "Excellent analysis — disposition, technique, and escalation all accurate.";
