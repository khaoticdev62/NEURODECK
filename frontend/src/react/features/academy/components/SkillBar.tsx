  ...
  return (
    <div className={`flex-1 ${compact ? 'space-y-0.5' : 'space-y-1.5'}`}>
      <div className="flex items-center justify-between">
        <span className={`${compact ? 'text-[11px]' : 'text-xs'} font-medium text-nd-text-secondary`}>
          {label}
        </span>
        <span className={`${compact ? 'text-[11px]' : 'text-xs'} font-mono font-semibold text-nd-text-primary`}>
          {pct}
        </span>
      </div>
      <div
