    ...
    <section className="flex min-h-0 flex-col gap-3 overflow-auto rounded-2xl border border-nd-text-muted/15 bg-nd-surface/30 p-3" tabIndex={0} aria-label="Terminal diagnostics">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs text-nd-text-muted">
        <div className="rounded-2xl border border-nd-text-muted/15 bg-nd-surface/40 p-3">
          <div className="font-semibold text-nd-text">Active sessions</div>
          <div className="mt-1">{diagnostics?.activeSessionCount ?? 0}</div>
        </div>
        <div className="rounded-2xl border border-nd-text-muted/15 bg-nd-surface/40 p-3">
          <div className="font-semibold text-nd-text">Platform</div>
          <div className="mt-1">{environment?.platform ?? "unknown"}</div>
        </div>
        <div className="rounded-2xl border border-nd-text-muted/15 bg-nd-surface/40 p-3">
          <div className="font-semibold text-nd-text">Shell</div>
      ...
      <div className="rounded-2xl border border-nd-text-muted/15 bg-nd-surface/40 p-3 text-xs text-nd-text-muted">
        <div className="font-semibold text-nd-text">Tool probes</div>
        <div className="mt-2 grid grid-cols-2 gap-2">
          {(environment?.probes ?? []).slice(0, 8).map((probe) => (
            <div key={probe.name} className="rounded-xl border border-nd-text-muted/15 bg-nd-bg/40 p-2">
              <div className="font-semibold text-nd-text">{probe.name}</div>
