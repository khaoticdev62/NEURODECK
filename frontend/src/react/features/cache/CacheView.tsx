...
export function CacheView({ state }: { state: NeuroDeckState }) {
  return (
    <Panel eyebrow="Offline Cache" title="Local Readiness Center" className="h-full overflow-hidden">
      {state.cacheEntries.length === 0 ? (
        <EmptyState icon={HardDrive} title="No cached items" description="Cached models, docs, and sessions will appear here." />
      ) : (
        <div className="grid gap-4 overflow-y-auto p-4 scrollbar-thin lg:grid-cols-2">
          {state.cacheEntries.map((entry) => (
            <article key={entry.id} className="rounded-3xl border border-nd-text-muted/15 bg-nd-surface/40 p-4 transition hover:border-nd-accent/25">
              ...
              <button type="button" className="mt-4 inline-flex items-center gap-2 rounded-xl border border-nd-text-muted/15 bg-nd-surface/40 px-3 py-2 text-xs text-nd-text-muted transition hover:border-nd-accent/25 hover:text-nd-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nd-accent/40">
                <RotateCcw className="h-4 w-4" aria-hidden="true" /> Queue Refresh
              </button>
            </article>
          ))}
        </div>
