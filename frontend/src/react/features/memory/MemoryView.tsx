        ...
        {filtered.map((memory) => (
          <article key={memory.id} className={`rounded-3xl border p-4 transition ${memory.pinned ? 'border-nd-accent/30 bg-nd-accent/[0.055]' : 'border-nd-text-muted/15 bg-nd-surface/40 hover:border-nd-accent/25'}`}>
            <div className="flex items-start justify-between gap-3">
              ...
              <button type="button" onClick={() => dispatch({ type: 'toggle-memory-pin', id: memory.id })} className={`rounded-xl border px-2.5 py-2 text-xs transition ${memory.pinned ? 'border-nd-accent/30 bg-nd-accent/10 text-nd-accent' : 'border-nd-text-muted/15 text-nd-text0 hover:text-nd-text'}`}>
