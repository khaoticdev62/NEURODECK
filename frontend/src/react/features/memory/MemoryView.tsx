...
export function MemoryView({
  ...
  const handleRestore = useCallback(
    async (name: string) => {
      if (
        !confirm(`Restore backup "${name}"? This will replace your current memory with the backup.`)
      )
        return;
      setBusy("restore");
      try {
  ...
  return (
    <Panel
      ...
      className="memory-shell !flex-col h-full overflow-hidden"
    >
              ...
              {backups.map((b) => (
                <li key={b.name} className="flex items-center gap-3 px-4 py-2.5">
                  <Archive
                    ...
                    aria-hidden="true"
                  />
                  <span className="min-w-0 flex-1 truncate font-mono text-xs text-nd-text/80">
                    {b.name}
                  </span>
