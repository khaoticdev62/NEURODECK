...
describe('Tooltip', () => {
  it('renders children', () => {
    render(<Tooltip label="More info"><button>Help</button></Tooltip>);
  ...
  it('renders tooltip label text in the DOM (hidden by default via opacity-0)', () => {
    render(<Tooltip label="Keyboard shortcut: Ctrl+K"><button>Search</button></Tooltip>);
    expect(screen.getByText('Keyboard shortcut: Ctrl+K')).toBeDefined();
  });

  it('tooltip label span has pointer-events-none', () => {
    render(<Tooltip label="hint"><span>target</span></Tooltip>);
  ...
  it('wraps children in a relative span for positioning', () => {
    const { container } = render(<Tooltip label="x"><button>btn</button></Tooltip>);
