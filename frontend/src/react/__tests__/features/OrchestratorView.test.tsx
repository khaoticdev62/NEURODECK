...
const { mockList, mockLoad, mockImport, mockExport, mockSave, mockDelete, mockRun, mockStop } = vi.hoisted(() => ({
  mockList: vi.fn(),
  mockLoad: vi.fn(),
  mockImport: vi.fn(),
  mockExport: vi.fn(),
...
describe('OrchestratorView', () => {
  ...
  it('calls workflow.importJson with pasted JSON and closes modal', async () => {
    ...
    fireEvent.change(textarea, { target: { value: '{"name":"my-workflow","nodes":[],"edges":[]}' } });
    const dialog = screen.getByRole('dialog');
    const importBtn = within(dialog).getByRole('button', { name: /^import$/i });
