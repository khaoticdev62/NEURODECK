...
import { StepEnvironment, type InstallerProgressMap } from '../../../components/onboarding/steps/StepEnvironment';

const HEALTHY_DIAG = {
  pty_ok: true, pty_details: 'PTY ready',
  network_ok: true, network_details: 'Internet connected',
  keychain_ok: true, keychain_details: 'Keychain active',
  audio_ok: true, audio_details: 'Microphone detected',
  ssh_ok: true, ssh_details: 'OpenSSH ready',
  tts_ok: true, tts_details: 'espeak-ng ready',
};
...
describe('StepEnvironment', () => {
  ...
  it('shows loading spinner when diagnosticsLoading=true', () => {
    render(<StepEnvironment {...DEFAULT_PROPS} diagnosticsLoading={true} diagnosticResult={null} />);
  ...
  it('calls onInstall with "ssh" when install button is clicked', async () => {
    ...
    const missingDiag = { ...HEALTHY_DIAG, ssh_ok: false, ssh_details: 'OpenSSH not found' };
    render(<StepEnvironment {...DEFAULT_PROPS} diagnosticResult={missingDiag} onInstall={onInstall} />);
  ...
  it('shows download progress bar for downloading state', () => {
    ...
    render(<StepEnvironment {...DEFAULT_PROPS} ollamaInstalled={false} installerProgress={progressWithDownload} />);
  ...
  it('shows Cancel button during install', () => {
    ...
    render(<StepEnvironment {...DEFAULT_PROPS} ollamaInstalled={false} installerProgress={progressWithInstall} />);
  ...
  it('shows Retry button on install failure', () => {
    ...
    render(<StepEnvironment {...DEFAULT_PROPS} ollamaInstalled={false} installerProgress={progressWithFail} />);
