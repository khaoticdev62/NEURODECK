...
describe('StepNpmInstaller', () => {
  ...
  it('shows a truthful blocked state when node and npm are unavailable', async () => {
    ...
    expect(screen.getByRole('button', { name: /install npm package/i }).hasAttribute('disabled')).toBe(true);
  });
  ...
  it('installs a manually entered package and optional version', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/Package/i), '@modelcontextprotocol/server-filesystem');
    await userEvent.type(screen.getByLabelText(/Version/i), '1.2.3');
    await userEvent.click(screen.getByRole('button', { name: /install @modelcontextprotocol\/server-filesystem/i }));
