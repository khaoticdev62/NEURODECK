...
describe('StepModels', () => {
  ...
  it('API key input type is password by default', () => {
    render(<StepModels {...DEFAULT_PROPS} providerType="openai_compat" apiKey="sk-test" showApiKey={false} />);
  ...
  it('API key input type is text when showApiKey=true', () => {
    render(<StepModels {...DEFAULT_PROPS} providerType="openai_compat" apiKey="sk-test" showApiKey={true} />);
    const input = screen.getByLabelText(/api token/i) as HTMLInputElement;
  ...
  it('calls onToggleShowApiKey when visibility button is clicked', async () => {
    ...
    render(<StepModels {...DEFAULT_PROPS} providerType="openai_compat" onToggleShowApiKey={onToggleShowApiKey} />);
  ...
  it('renders success test result', () => {
    render(<StepModels {...DEFAULT_PROPS} testResult={{ success: true, message: 'Connected to Ollama. Found models: llama3' }} />);
    expect(screen.getByText(/connected to ollama/i)).toBeDefined();
  });

  it('renders failure test result', () => {
    render(<StepModels {...DEFAULT_PROPS} testResult={{ success: false, message: 'Connection timed out' }} />);
