...
describe('SchedulerView', () => {
  ...
  it('Add Task button is disabled when name is filled but cron is cleared', async () => {
    ...
    fireEvent.change(screen.getByRole('textbox', { name: /cron expression/i }), { target: { value: '' } });
  ...
  it('calls scheduler.add and refreshes list when form submitted', async () => {
    ...
    fireEvent.change(screen.getByRole('textbox', { name: /cron expression/i }), { target: { value: '0 9 * * MON' } });
    await userEvent.click(screen.getByRole('button', { name: /add task/i }));

    expect(mockAdd).toHaveBeenCalledWith(expect.objectContaining({ name: 'Morning Report', cron: '0 9 * * MON' }));
