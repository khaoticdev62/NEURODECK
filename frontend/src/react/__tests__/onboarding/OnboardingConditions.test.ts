...
import { buildOnboardingSteps, getCoreOnboardingManifests } from '../../onboarding/OnboardingRegistry';
...
describe('Condition evaluation', () => {
  ...
  it('contextual mode returns steps for the requested view', () => {
    const steps = buildOnboardingSteps({ mode: 'contextual', state: emptyState, activeView: 'memory' });
    expect(steps.some((s) => s.featureId === 'memory')).toBe(true);
  });

  it('contextual mode for terminal returns terminal steps', () => {
    const steps = buildOnboardingSteps({ mode: 'contextual', state: emptyState, activeView: 'terminal' });
...
describe('validateOnboardingManifest', () => {
  const validManifest: OnboardingManifest = {
    ...
    priority: 50,
    category: 'tool',
    anchors: [{ id: 'test-anchor', selector: '[data-onboarding-anchor="test"]', fallbackLabel: 'test' }],
    steps: [
      {
  ...
  it('step with invalid animation type produces error', () => {
    const errors = validateOnboardingManifest({
      ...validManifest,
      steps: [{ ...validManifest.steps[0], animation: { type: 'mp4' as never, label: 'x', caption: 'y' } }],
    });
    expect(errors.length).toBeGreaterThan(0);
  });
