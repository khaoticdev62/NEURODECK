...
describe('OnboardingRegistry', () => {
  it('ships only valid core manifests', () => {
    ...
    expect(manifests.flatMap((manifest) => manifest.anchors).some((anchor) => anchor.id === 'command-palette-trigger')).toBe(true);
  });
  ...
  it('returns contextual help for the active view', () => {
    const steps = buildOnboardingSteps({ mode: 'contextual', state: baseState, activeView: 'terminal' });
