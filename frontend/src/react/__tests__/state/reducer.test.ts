    ...
    case "toggle-agent":
      return {
        ...
        agents: state.agents.map((a) =>
          a.id === action.id
            ? { ...a, status: nextStatus(a.status), lastAction: "Status toggled locally" }
            : a
        ),
      };
        ...
        agents: state.agents.map((a) =>
          a.id === action.id
            ? {
                ...
                lastAction: action.lastAction ?? a.lastAction,
                task: action.task ?? a.task,
              }
            : a
        ),
      };
    ...
    case "toggle-memory-pin":
      return {
        ...
        memories: state.memories.map((mem) =>
          mem.id === action.id ? { ...mem, pinned: !mem.pinned } : mem
        ),
      };
    case "set-memories":
...
describe("reducer — hydrate", () => {
  it("sets hydrated=true and clears transient fields", () => {
    s = {
      ...
      busyLabel: "loading",
      commandOpen: true,
      lastError: { title: "E", message: "oops" },
    };
...
describe("reducer — agents", () => {
  it("toggle-agent: idle → thinking", () => {
    ...
    s = {
      ...s,
      agents: s.agents.map((a) => (a.id === id ? { ...a, status: "idle" as AgentStatus } : a)),
    };
  ...
  it("toggle-agent: thinking → complete", () => {
    ...
    s = {
      ...s,
      agents: s.agents.map((a) => (a.id === id ? { ...a, status: "thinking" as AgentStatus } : a)),
    };
    const next = dispatch({ type: "toggle-agent", id });
  ...
  it("toggle-agent: complete → idle", () => {
    ...
    s = {
      ...s,
      agents: s.agents.map((a) => (a.id === id ? { ...a, status: "complete" as AgentStatus } : a)),
    };
    const next = dispatch({ type: "toggle-agent", id });
...
describe("reducer — plugins", () => {
  ...
  it("set-plugins: replaces plugins array", () => {
    const newPlugins = [
      { id: "p1", name: "N", description: "D", status: "enabled" as const, permissions: [] },
    ];
...
describe("reducer — messages", () => {
  ...
  it("append-message: caps messages array at 80 entries", () => {
    ...
    const first = dispatch({
      type: "append-message",
      message: { id: "x80", role: "user", content: "a", createdAt: now },
    });
    expect(first.messages.length).toBe(80);
    const second = reduce(first, {
      type: "append-message",
      message: { id: "x81", role: "user", content: "b", createdAt: now },
    });
...
describe("reducer — reset", () => {
  it("reset-local-state: returns initial state with hydrated=true", () => {
    s = {
      ...
      selectedTheme: "Blacksite",
      busyLabel: "busy",
      lastError: { title: "E", message: "fail" },
      deckMode: true,
    };
