  ...
  switch (tool) {
    ...
    case "computer_mouse_move": {
      ...
      if (!Number.isFinite(x) || !Number.isFinite(y))
        throw new Error("computer_mouse_move requires numeric args.x and args.y.");
    ...
    case "computer_mouse_click": {
      await computer.click(String(args.button || "left"));
      return `${args.button || "left"} mouse click sent.`;
    }
    case "computer_type": {
      const text = String(args.text || "");
...
function _btRequire(args, ...keys) {
  for (const k of keys) {
    if (!String(args[k] || args[k.replace(/_([a-z])/g, (_, c) => c.toUpperCase())] || ""))
      throw new Error(`${keys[0]}'s tool requires args.${k}.`);
  }
}

function _btSessionId(args) {
  return String(args.session_id || args.sessionId || "");
}
...
async function _btNavigateSession(args) {
  const sessionId = _btSessionId(args),
    url = String(args.url || "");
  if (!sessionId || !url)
    throw new Error("browser_navigate_session requires args.session_id and args.url.");
...
async function _btClick(args) {
  ...
  if (!sessionId || !selector)
    throw new Error("browser_click requires args.session_id and args.selector.");
...
async function _btFill(args) {
  ...
  if (!sessionId || !selector)
    throw new Error("browser_fill requires args.session_id and args.selector.");
...
async function _btEvaluateJs(args) {
  ...
  if (!sessionId || !script)
    throw new Error("browser_evaluate_js requires args.session_id and args.script.");
...
function _agCleanupRtListeners(ag) {
  if (ag.rtUnlistenChunk) {
    ag.rtUnlistenChunk();
    ag.rtUnlistenChunk = null;
  }
  if (ag.rtUnlistenDone) {
    ag.rtUnlistenDone();
    ag.rtUnlistenDone = null;
  }
}

function _agCleanupOrchestratorListeners(ag) {
  if (ag.orchUnlistenPlan) {
    ag.orchUnlistenPlan();
    ag.orchUnlistenPlan = null;
  }
  if (ag.orchUnlistenTaskStart) {
    ag.orchUnlistenTaskStart();
    ag.orchUnlistenTaskStart = null;
  }
  if (ag.orchUnlistenTaskDone) {
    ag.orchUnlistenTaskDone();
    ag.orchUnlistenTaskDone = null;
  }
  if (ag.orchUnlistenComplete) {
    ag.orchUnlistenComplete();
    ag.orchUnlistenComplete = null;
  }
  if (ag.orchestratorPollTimer) {
    clearInterval(ag.orchestratorPollTimer);
    ag.orchestratorPollTimer = null;
  }
}
...
function _agSetMode(mode, ag) {
  ...
  document
    .getElementById("agent-toolbar-roundtable")
    ?.classList.toggle("hidden", mode !== "roundtable");
  ...
  if (ag.codePaneTitle)
    ag.codePaneTitle.textContent = mode === "orchestrate" ? "Orchestration Plan" : "Current Code";
  if (ag.outputTitle)
    ag.outputTitle.textContent = mode === "orchestrate" ? "Orchestration Status" : "Output";
  if (mode === "orchestrate") {
    _agRefreshOrchestratorStatus(ag).catch(() => {});
  } else if (ag.codePre) {
    ag.codePre.textContent = ag.lastCode || "";
  }
}
  ...
  if (mode === "roundtable") {
    invoke("get_personas")
      .then((p) => {
        ...
        _agPopulateRtPersonas(ag);
      })
      .catch(() => _agPopulateRtPersonas(ag));
  }
  if (mode === "orchestrate") {
    _agRefreshOrchestratorStatus(ag).catch(() => {});
  }
...
function _agRenderOrchestratorStatus(status = {}, ag) {
  ...
  if (running) ag.outputEl.textContent = `Orchestrator running: ${goal || "awaiting goal"}`;
    ...
    ag.outputEl.textContent = `Orchestrator idle. ${tasks.length} task(s) in the current plan.`;
...
async function _agWireOrchestratorListeners(goal, ag) {
  ag.orchUnlistenPlan = await listen("orchestrator_plan_ready", (event) => {
    ...
    _agRenderOrchestratorStatus(
      { running: true, goal: plan.goal || goal, tasks: plan.tasks || [] },
      ag
    );
  ...
  ag.orchUnlistenTaskDone = await listen("agent_task_done", (event) => {
    const p = event.payload || {};
    if (p.error) {
      _agAppendLog(ag.logEl, "error", `${p.role || "Agent"} failed: ${p.error}`);
    } else {
      _agAppendLog(
        ag.logEl,
        "done",
        `${p.role || "Agent"} completed: ${p.result ? String(p.result).slice(0, 180) : "done"}`
      );
    }
    _agRefreshOrchestratorStatus(ag).catch(() => {});
  });
...
async function _agStartOrchestration(ag) {
  ...
  if (!goal) {
    ag.orchestratorGoalInput.focus();
    return;
  }
  _agCleanupOrchestratorListeners(ag);
...
async function _agStartRoundtable(ag) {
  const pA = ag.rtPersonaA.value.trim(),
    pB = ag.rtPersonaB.value.trim();
  ...
  invoke("send_command", { message: `/discuss ${pA} ${pB} ${topic} rounds:${rounds}` }).catch(
    (error) => {
...
async function _agHandleToolStep(parsed, step, ag, history) {
  ...
  _agAppendLog(
    ag.logEl,
    "exec",
    `Requesting ${isBrowser ? "browser" : "computer"} tool: ${tool}`,
    step
  );
  let toolOut;
  try {
    toolOut = isBrowser
      ? await _agRunBrowserTool(tool, parsed.args || {})
      : await _agRunComputerTool(tool, parsed.args || {});
  } catch (e) {
    toolOut = `${prefix} ${e}`;
  }
...
async function _agHandleCodeStep(parsed, step, ag, history) {
  ...
  _agAppendLog(
    ag.logEl,
    "code",
    `[${(parsed.lang || "?").toUpperCase()}] ${parsed.summary || ""}`,
    step
  );
  ...
  _agAppendLog(
    ag.logEl,
    "output",
    execOut.length > 300 ? execOut.slice(0, 300) + "…" : execOut,
    step
  );
  history.push({ role: "step", content: JSON.stringify(parsed) });
...
async function _agRunLoop(task, ag) {
  ...
  try {
    ...
    if (registry) {
      const profile =
        (registry.profiles || []).find((p) => p.id === activeAgent) ||
      ...
      if (missing.length > 0) {
        _agAppendLog(
          ag.logEl,
          "info",
          `⚠️ Agent "${profile?.name || activeAgent}" lacks: ${missing.join(", ")}. Some features may fail.`,
          0
        );
      }
    }
  ...
  for (let step = 1; step <= MAX_STEPS; step++) {
    if (ag.agentShouldStop) {
      _agAppendLog(ag.logEl, "info", "Agent stopped by user.", step);
      break;
    }
    ...
    try {
      ...
      _agAppendLog(ag.logEl, "error", `LLM call failed: ${e}`, step);
      break;
    }
    ...
    if (parsed.action === "done") {
      _agAppendLog(ag.logEl, "done", parsed.summary || "Task complete.", step);
      break;
    }
    if (parsed.action === "error") {
      _agAppendLog(ag.logEl, "error", parsed.summary || "Agent reported an error.", step);
      break;
    }
    if (parsed.action === "computer" || parsed.action === "browser") {
      await _agHandleToolStep(parsed, step, ag, history);
      continue;
    }
    if (!parsed.code) {
      _agAppendLog(ag.logEl, "error", "Agent returned no code and action is not done.", step);
      break;
    }
    if (ag.agentShouldStop) break;
...
function _agWireAllEvents(ag) {
  ...
  ag.taskInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter" && !e.shiftKey && !ag.agentRunning) {
      e.preventDefault();
      ag.runBtn.click();
    }
  });
  ag.stopBtn.onclick = () => {
    ag.agentShouldStop = true;
  };
  ...
  ag.modeBtns.forEach((btn) =>
    btn.addEventListener("click", () => _agSyncModeUI(btn.dataset.mode, ag))
  );
  ag.rtStartBtn.addEventListener("click", () => {
    if (!ag.rtRunning) _agStartRoundtable(ag);
  });
  ...
  ag.orchestratorStartBtn.addEventListener("click", () => {
    if (!ag.orchestratorRunning) _agStartOrchestration(ag);
  });
  ag.orchestratorStatusBtn.addEventListener("click", () => {
    _agRefreshOrchestratorStatus(ag).catch(() => {});
  });
  ag.orchestratorStopBtn.addEventListener("click", () => {
    _agStopOrchestration(ag).catch(() => {});
  });
}
...
export function initAgentView() {
  ...
  const ag = {
    ...
    lastCode: "",
    lastLang: "python",
    lastOrchestratorGoal: "",
    rtRunning: false,
    rtUnlistenChunk: null,
    rtUnlistenDone: null,
    orchUnlistenPlan: null,
    orchUnlistenTaskStart: null,
    orchUnlistenTaskDone: null,
    orchUnlistenComplete: null,
  };
  _agWireAllEvents(ag);
