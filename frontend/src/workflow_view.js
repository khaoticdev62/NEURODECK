...
const NODE_TYPES = {
  trigger: {
    ...
    icon: "▶",
    color: "#22c55e",
    desc: "Start the workflow — manual, cron, or on-event",
  ...
  prompt: {
    ...
    icon: "🤖",
    color: "#00f0ff",
    desc: "Send a prompt to the LLM; {{input}} = upstream output",
  ...
  shell: {
    ...
    icon: "💻",
    color: "#f59e0b",
    desc: "Run a shell command; $INPUT holds upstream output",
    ports: { in: true, out: true },
  },
  file_op: {
    label: "File Op",
  ...
  pty_cmd: {
    ...
    icon: "⌨",
    color: "#fb923c",
    desc: "Send a command to the active terminal session",
    ports: { in: true, out: true },
  },
  memory: {
    label: "Memory Search",
    icon: "🧠",
    color: "#a78bfa",
    desc: "Search vector DB with {{input}} as the query",
    ports: { in: true, out: true },
  },
  condition: {
    label: "Condition",
    icon: "⑂",
    color: "#facc15",
    desc: "Branch on a JS expression — true path or false path",
    ports: { in: true, outTrue: true, outFalse: true },
  },
  transform: {
    label: "Transform",
    icon: "⚙",
    color: "#fb923c",
    desc: "Reshape text: trim, case, extract, template",
    ports: { in: true, out: true },
  },
  output: {
    label: "Output",
    icon: "📤",
    color: "#64748b",
    desc: "Display final result in the run log",
...
function _wfBuildHtml() {
  const paletteItems = Object.entries(NODE_TYPES)
    .map(
      ([type, def]) =>
...
function _wireToolbar() {
  _nameEl.addEventListener("input", () => {
    _s.workflowName = _nameEl.value;
  });
  ...
  document.getElementById("wf-stop-btn").addEventListener("click", () => {
    _s.running = false;
  });
...
function _wireCanvas() {
  ...
  _canvasEl.addEventListener("mousedown", (e) => {
    if (
      ...
      !e.target.classList.contains("wf-canvas-hint")
    )
      return;
...
function _renderNodes() {
  ...
  for (const node of _s.nodes) {
    ...
    div.addEventListener("mousedown", (e) => {
      if (e.target.classList.contains("wf-port") || e.target.classList.contains("wf-node-del"))
        return;
      e.stopPropagation();
...
function _nodePreview(node) {
  ...
  switch (node.type) {
    case "trigger":
      return `<span class="wf-preview">${_esc(_triggerTypeLabel(c))} · ${_esc((c.seed || "").slice(0, 30))}</span>`;
    case "prompt":
      return `<span class="wf-preview">${_esc((c.prompt || "").slice(0, 40))}</span>`;
    case "shell":
      return `<span class="wf-preview">${_esc((c.command || "").slice(0, 40))}</span>`;
    case "file_op":
      return `<span class="wf-preview">${c.mode}: ${_esc((c.path || "").slice(0, 35))}</span>`;
    case "pty_cmd":
      return `<span class="wf-preview">${_esc((c.command || "").slice(0, 40))}</span>`;
    case "memory":
      return `<span class="wf-preview">query: ${_esc((c.query || "").slice(0, 30))}</span>`;
    case "condition":
      return `<span class="wf-preview">${_esc((c.expression || "").slice(0, 40))}</span>`;
    case "transform":
      return `<span class="wf-preview">mode: ${_esc(c.mode || "trim")}</span>`;
    case "output":
      return `<span class="wf-preview">displays result</span>`;
    default:
      return "";
  }
}
...
function _getPortPos(nodeId, port) {
  ...
  switch (port) {
    case "in":
      return { x: ox, y: oy + nh / 2 };
    case "out":
      return { x: ox + nw, y: oy + nh / 2 };
    case "out-true":
      return { x: ox + nw, y: oy + nh * 0.35 };
    case "out-false":
      return { x: ox + nw, y: oy + nh * 0.65 };
    default:
      return { x: ox + nw, y: oy + nh / 2 };
  }
}
...
function _renderEdges() {
  ...
  for (const edge of _s.edges) {
    ...
    if (edge.fromPort === "out-true") {
      cls += " wf-edge-true";
      markerId = "#wf-arrow-true";
    }
    if (edge.fromPort === "out-false") {
      cls += " wf-edge-false";
      markerId = "#wf-arrow-false";
    }
    ...
    path.addEventListener("keydown", (e) => {
      if (e.key === "Delete" || e.key === "Backspace") delEdge();
    });
    ...
    if (edge.fromPort === "out-true" || edge.fromPort === "out-false") {
      ...
      text.setAttribute(
        "class",
        edge.fromPort === "out-true" ? "wf-edge-label-true" : "wf-edge-label-false"
      );
...
function _wfPropFields(node) {
  ...
  switch (node.type) {
    case "trigger":
      return _wfPropTrigger(c);
    case "prompt":
      return _wfPropPrompt(c);
    case "shell":
      return _wfPropShell(c);
    case "file_op":
      return _wfPropFileOp(c);
    case "pty_cmd":
      return _wfPropPtyCmd(c);
    case "memory":
      return _wfPropMemory(c);
    case "condition":
      return _wfPropCondition(c);
    case "transform":
      return _wfPropTransform(c);
    case "output":
      return _wfPropOutput();
    default:
      return "";
  }
}
...
function _wfWirePropInputs(node) {
  _propEl.querySelectorAll(".wf-prop-input, .wf-prop-select").forEach((el) => {
    el.addEventListener("input", () => {
      node.config[el.dataset.key] =
        el.tagName === "INPUT" && el.type === "number" ? Number(el.value) : el.value;
...
function _updatePropPanel(node) {
  if (!_propEl) return;
  if (!node) {
    _propEl.innerHTML = '<div class="wf-props-empty">Select a node to edit its properties.</div>';
    return;
  }
  const def = NODE_TYPES[node.type];
  _propEl.innerHTML = `
...
async function _runWorkflow() {
  ...
  if (!_s.nodes.length) {
    _logLine("No nodes in workflow.", "warn");
    return;
  }

  const trigger = _s.nodes.find((n) => n.type === "trigger");
  if (!trigger) {
    _logLine("No Trigger node found.", "error");
    return;
  }
...
function _runTransformNode(c, input, resolved) {
  switch (c.mode) {
    ...
    case "template":
      return resolved(c.template || "{{input}}");
    case "extract_json":
      try {
...
async function _runNode(node, input) {
  ...
  switch (node.type) {
    case "trigger":
      return c.seed || "";
    case "prompt":
      return invoke("llm_oneshot", { prompt: resolved(c.prompt), maxTokens: 800 });
    case "shell":
      return invoke("agent_exec_code", { code: resolved(c.command), lang: c.lang || "bash" });
    case "file_op":
      return _runFileOpNode(c, resolved);
    case "pty_cmd": {
      const cmd = resolved(c.command);
      const sid = c.session || "main_pty_session";
      await invoke("pty_write", { id: sid, data: cmd + "\n" });
      return `Sent to PTY [${sid}]: ${cmd}`;
    }
    case "memory":
      return _runMemoryNode(c, resolved);
    case "condition":
      return input;
    case "transform":
      return _runTransformNode(c, input, resolved);
    case "output":
...
async function _saveWorkflow() {
  ...
  const json = JSON.stringify(
    { name, nodes: _s.nodes.map(_stripRunState), edges: _s.edges, uid: _uid },
    null,
    2
  );
  try {
...
async function _loadSaved() {
  ...
  try {
    ...
    if (!names.length) {
      listEl.innerHTML = '<span class="wf-saved-empty">No saved workflows</span>';
      return;
    }
    listEl.innerHTML = names
      ...
      .join("");
    listEl
      .querySelectorAll(".wf-saved-load")
      .forEach((btn) => btn.addEventListener("click", () => _loadWorkflow(btn.dataset.name)));
...
function _evalCondition(expr, input) {
  ...
  // isEmpty / isNotEmpty
  if (s === "isEmpty" || s === "input.length === 0" || s === 'input === ""')
    return input.trim() === "";
  if (s === "isNotEmpty" || s === "input.length > 0" || s === 'input !== ""')
    return input.trim() !== "";
  ...
  if (regexM) {
    try {
      return new RegExp(regexM[1], regexM[2]).test(input);
    } catch {
      return false;
    }
  }
