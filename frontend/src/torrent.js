...
const TORRENT_REFRESH_INTERVAL_MS = 2000;
...
function torrentInsight(entry) {
  if (!entry.metadata_known)
    return "Metadata has not resolved yet. Magnet bootstrap is still in progress.";
  if (entry.completed && entry.paused)
    return "Payload is complete and safely parked. Resume only if you want to seed.";
  ...
  if (Number(entry.peers || 0) === 0)
    return "No active peers detected. The swarm may be cold or trackers may be lagging.";
  if (Number(entry.peers || 0) <= 2)
    return "Low peer availability. Expect unstable throughput until more peers appear.";
  if (Number(entry.download_rate_bps || 0) === 0 && Number(entry.bytes_remaining || 0) > 0)
    return "Swarm is connected but not moving data yet. Watch peer quality and tracker churn.";
  if (clampPercent(entry.progress_pct) >= 95)
    return "Final pieces are in flight. Completion should be close if the swarm stays healthy.";
  if (Number(entry.trackers || 0) === 0)
    return "Tracker count is empty. This swarm may be relying on magnet metadata and peer discovery only.";
...
function switchSharePanel(panel) {
  ...
  document
    ...
    .forEach((section) =>
      section.classList.toggle("active", section.id === `share-panel-${panel}`)
    );
}
...
function handoffTorrentToLan(entry) {
  ...
  if (!savePath) {
    addNotification?.(
      "Payload Pending",
      "This torrent does not have a resolved save path yet.",
      "warn"
    );
    return;
  }
  ...
  if (!pathInput) {
    addNotification?.(
      "LAN Share Unavailable",
      "The LAN transfer path field is not available right now.",
      "error"
    );
    return;
  }
  ...
  addNotification?.(
    "LAN Handoff Ready",
    "Payload path staged in the LAN transfer panel.",
    "success"
  );
}
...
function findDuplicateTorrent(source) {
  ...
  const exactMatch = torrentUi.items.find((entry) =>
    eqIgnoreCase(entry.source_value, normalizedSource)
  );
...
function confirmTorrentRemoval(entry, deleteData) {
  const mode = deleteData
    ? "remove this torrent and delete its downloaded payload from the managed root"
...
function _buildInspectorHero(entry) {
  ...
  heroText.append(nameEl, sourceMeta);
  const heroStatus = document.createElement("div");
  heroStatus.className = "torrent-inspector-status-stack";
  const statusEl = document.createElement("span");
  statusEl.className = `transfer-status torrent-status ${torrentStatusClass(entry)}`;
  statusEl.textContent = torrentStatusLabel(entry);
  const pct = document.createElement("strong");
  pct.className = "torrent-inspector-progress";
  pct.textContent = `${clampPercent(entry.progress_pct).toFixed(1)}%`;
  heroStatus.append(statusEl, pct);
  hero.append(heroText, heroStatus);
  return hero;
}

function _buildInspectorSections(entry) {
  ...
  sourceBlock.textContent =
    entry.source_value || entry.source_display || "No source string available.";
  source.appendChild(sourceBlock);
  return [overview, swarm, source];
}

function _buildInspectorActions(entry) {
  const section = buildInspectorSection("Actions");
  const actionGrid = document.createElement("div");
  actionGrid.className = "torrent-inspector-actions";
  const primary = entry.paused
    ? torrentActionButton("Resume", "play", () =>
        runTorrentAction("torrent_resume", { id: entry.id }, "Torrent resumed.")
      )
    : torrentActionButton("Pause", "pause", () =>
        runTorrentAction("torrent_pause", { id: entry.id }, "Torrent paused.")
      );
  actionGrid.append(
    primary,
    torrentActionButton("Copy Hash", "copy", () =>
      writeToClipboard(entry.info_hash || "", "Info hash")
    ),
    torrentActionButton("Copy Source", "clipboard", () =>
      writeToClipboard(entry.source_value || "", "Source")
    ),
    torrentActionButton("Open Root", "folderOpen", openTorrentRoot, "ghost"),
    torrentActionButton(
      "Reveal Payload",
      "arrowUpRight",
      async () => {
    ...
    torrentActionButton("Send to LAN", "share2", () => handoffTorrentToLan(entry), "ghost"),
    torrentActionButton(
      "Remove",
      "trash2",
      async () => {
    ...
    torrentActionButton(
      ...
      async () => {
        ...
        await runTorrentAction(
          "torrent_remove",
          { id: entry.id, deleteData: true },
          "Torrent removed and payload deleted."
        );
      },
      "danger"
    )
...
function renderInspector(entry) {
  ...
  if (!entry) {
    ...
    empty.textContent =
      "Select a queue item to inspect swarm posture, source lineage, and next actions.";
    torrentUi.inspectorEl.appendChild(empty);
    return;
  }
  torrentUi.inspectorEl.appendChild(_buildInspectorHero(entry));
  ...
  _buildInspectorSections(entry).forEach((s) => torrentUi.inspectorEl.appendChild(s));
  torrentUi.inspectorEl.appendChild(_buildInspectorActions(entry));
}
...
function _trBuildSelectorCell(entry) {
...
function _trBuildQueueCell(entry) {
...
function _trBuildStatusCell(entry) {
...
function _trBuildProgressCell(entry) {
...
function _trBuildActionsCell(entry) {
  ...
  const copyHash = torrentActionButton(
    "Hash",
    "copy",
    async (event) => {
      event.stopPropagation();
  ...
  const inspect = torrentActionButton(
    "Inspect",
    "info",
    (event) => {
      event.stopPropagation();
  ...
  const reveal = torrentActionButton(
    "Reveal",
    "arrowUpRight",
    async (event) => {
      event.stopPropagation();
  ...
  const remove = torrentActionButton(
    "Remove",
    "trash2",
    async (event) => {
      event.stopPropagation();
...
function renderTorrentRow(entry) {
  ...
  if (entry.id === torrentUi.selectedId) row.classList.add("active");
  if (torrentUi.selectedIds.has(entry.id)) row.classList.add("selected");
  row.tabIndex = 0;
  ...
  row.appendChild(_trBuildSelectorCell(entry));
  row.appendChild(_trBuildQueueCell(entry));
  row.appendChild(_trBuildStatusCell(entry));
  row.appendChild(_trBuildProgressCell(entry));
  ...
  row.appendChild(_trBuildActionsCell(entry));
  return row;
}
  ...
  try {
    ...
    torrentUi.items.forEach((entry) => {
      ...
      if (
        entry.completed &&
        !previous?.completed &&
        !torrentUi.completionNotifiedIds.has(entry.id)
      ) {
...
async function addTorrentFromInput() {
  ...
  if (!source) {
    addNotification?.(
      "Torrent Source Required",
      "Paste a magnet URI or a local .torrent path.",
      "warn"
    );
    return;
  }
  ...
  if (duplicate) {
    addNotification?.(
      "Duplicate Torrent Blocked",
      `${duplicate.name || duplicate.id} is already queued from the same source or info hash.`,
      "warn"
    );
  ...
  try {
    ...
    addNotification?.(
      "Torrent Added",
      "The torrent was validated and added paused by default.",
      "success"
    );
...
function attachDropzone() {
  ...
  torrentUi.dropzone.addEventListener("drop", async (event) => {
    ...
    if (
      droppedSource.toLowerCase().startsWith("magnet:") ||
      droppedSource.toLowerCase().endsWith(".torrent")
    ) {
...
async function runSelectedBatch(action) {
  ...
  if (!entries.length) {
    addNotification?.(
      "No Selection",
      "Select one or more torrents before running a batch action.",
      "warn"
    );
    return;
  }
  ...
  try {
    ...
    torrentUi.selectedIds = new Set(
      action.keepSelection
        ? entries.map((entry) => entry.id).filter((id) => torrentUi.selectedIds.has(id))
        : []
    );
...
function _tcInitDomRefs() {
  const $ = (id) => document.getElementById(id);
  torrentUi.addBtn = $("torrent-add-btn");
  torrentUi.refreshBtn = $("torrent-refresh-btn");
  torrentUi.pauseAllBtn = $("torrent-pause-all-btn");
  torrentUi.resumeAllBtn = $("torrent-resume-all-btn");
  torrentUi.openRootBtn = $("torrent-open-root-btn");
  torrentUi.sourceInput = $("torrent-source-input");
  torrentUi.dropzone = $("torrent-dropzone");
  torrentUi.searchInput = $("torrent-search-input");
  torrentUi.filterSelect = $("torrent-filter-select");
  torrentUi.sortSelect = $("torrent-sort-select");
  torrentUi.listEl = $("torrent-list");
  torrentUi.rootEl = $("torrent-root-label");
  torrentUi.countEl = $("torrent-count-label");
  torrentUi.inspectorEl = $("torrent-inspector");
  torrentUi.totalCountEl = $("torrent-total-count");
  torrentUi.runningCountEl = $("torrent-running-count");
  torrentUi.pausedCountEl = $("torrent-paused-count");
  torrentUi.completeCountEl = $("torrent-complete-count");
  torrentUi.batchCopyEl = $("torrent-batch-copy");
  torrentUi.selectVisibleBtn = $("torrent-select-visible-btn");
  torrentUi.clearSelectionBtn = $("torrent-clear-selection-btn");
  torrentUi.pauseSelectedBtn = $("torrent-pause-selected-btn");
  torrentUi.resumeSelectedBtn = $("torrent-resume-selected-btn");
  torrentUi.removeSelectedBtn = $("torrent-remove-selected-btn");
}

function _tcWireBatchBtns() {
  ...
  torrentUi.pauseSelectedBtn?.addEventListener("click", () =>
    runSelectedBatch({
      ...
      successMessage: (entries) =>
        `${entries.length} torrent${entries.length === 1 ? "" : "s"} paused.`,
      keepSelection: true,
    })
  );
  torrentUi.resumeSelectedBtn?.addEventListener("click", () =>
    runSelectedBatch({
      ...
      payload: (entry) => ({ id: entry.id }),
      successMessage: (entries) =>
        `${entries.length} torrent${entries.length === 1 ? "" : "s"} resumed.`,
      keepSelection: true,
    })
  );
  torrentUi.removeSelectedBtn?.addEventListener("click", () =>
    runSelectedBatch({
      ...
      confirmMessage: (entries) =>
        `Do you want to remove ${entries.length} selected torrent${entries.length === 1 ? "" : "s"} from the session?`,
      successMessage: (entries) =>
        `${entries.length} torrent${entries.length === 1 ? "" : "s"} removed from the session.`,
      keepSelection: false,
    })
  );
}
...
export function initTorrentClient() {
  ...
  torrentUi.pauseAllBtn?.addEventListener("click", () =>
    runTorrentAction("torrent_pause_all", {}, "All torrents paused.")
  );
  torrentUi.resumeAllBtn?.addEventListener("click", () =>
    runTorrentAction("torrent_resume_all", {}, "All torrents resumed.")
  );
  ...
  torrentUi.sourceInput.addEventListener("paste", () => {
    window.setTimeout(() => {
      ...
      if (duplicate)
        addNotification?.(
          "Duplicate Torrent Detected",
          `${duplicate.name || duplicate.id} already carries that magnet or info hash.`,
          "warn"
        );
    }, 0);
  });
  ...
  torrentUi.refreshTimer = window.setInterval(() => {
    ...
    if (shareView && shareView.classList.contains("active")) refreshTorrentPanel(true);
