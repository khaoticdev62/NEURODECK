...
function ResponseCard({ msg }: ResponseCardProps): React.ReactNode {
  ...
  return (
    <div style={{
      ...
      border: '1px solid', borderColor: isUser ? 'transparent' : 'var(--nd-border-subtle)',
      borderRadius: 'var(--nd-radius-lg)',
      boxShadow: isUser ? 'none' : 'var(--nd-elevation-panel)',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <RoleTag role={msg.role} model={msg.model} />
        {msg.streaming ? <StatusChip tone="info" size="sm" pulse>Streaming</StatusChip> : null}
        <span style={{ marginLeft: 'auto', fontFamily: 'var(--nd-font-mono)', fontSize: 10, color: 'var(--nd-text-muted)' }}>{msg.time}</span>
      </div>
      <div style={{ fontSize: 15, lineHeight: '22px', color: isUser ? 'var(--nd-text-primary)' : 'var(--nd-text-secondary)', whiteSpace: 'pre-wrap' }}>
        {msg.text}{msg.streaming ? <span className="nd-caret">▍</span> : null}
      </div>
      {!isUser && !msg.streaming ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}>
          <IconButton label="Copy" icon={<Copy size={15} />} size="sm" />
          <IconButton label="Regenerate" icon={<RefreshCw size={15} />} size="sm" />
          {msg.tokens ? <span style={{ marginLeft: 6, fontFamily: 'var(--nd-font-mono)', fontSize: 10, color: 'var(--nd-text-muted)' }}>{msg.tokens} tok · {msg.latency}</span> : null}
        </div>
      ) : null}
    </div>
  );
    ...
    <div ref={ref} style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
      {messages.length === 0 ? (
        <div style={{ margin: 'auto', textAlign: 'center', color: 'var(--nd-text-muted)' }}>
          <Terminal size={32} />
          <div style={{ marginTop: 12, fontSize: 15, color: 'var(--nd-text-secondary)' }}>Local intelligence layer, online.</div>
