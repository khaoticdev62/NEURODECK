...
### Every new Tauri command needs 3 things
1. `#[tauri::command]` function in a `src/` module
2. Added to `generate_handler![]` in `lib.rs`
3. Added to the mock IPC switch in `frontend/src/main.js`

Skip any of these and it silently fails in browser dev mode.
...
```
src-tauri/src/
  lib.rs          — All Tauri command handlers (~2800 lines)
...
frontend/src/
  main.js         — Entire frontend (~9500 lines, no framework)
  app.css         — Styles (~5000 lines)

infrastructure/
  src/secrets.rs  — OS keychain (keyring 2.x)
