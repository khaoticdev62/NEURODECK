  ...
  if (enabled) {
    return (
      <div className="grid h-full min-w-[76rem] grid-cols-[16rem_minmax(40rem,1fr)_18rem] gap-2 overflow-auto">
        ...
        <NdxEditorShell title="Kiosk Preferences">
          <div className="flex min-h-full min-w-0 flex-col gap-4 p-4">
            ...
            <p className="text-body text-text-secondary">
              Kiosk mode is active. Use the Exit Kiosk Mode control in the bottom-right corner with
              your real PIN to make changes here.
            </p>
          </div>
        </NdxEditorShell>
        <NdxToolWindow title="Exit Policy" subtitle="PIN required" side="right">
          <p className="text-meta text-text-secondary">
            Route restrictions remain active until the real Lock PIN exits kiosk mode.
          </p>
        </NdxToolWindow>
      </div>
    )
    ...
    <div className="grid h-full min-w-[76rem] grid-cols-[16rem_minmax(40rem,1fr)_18rem] gap-2 overflow-auto">
      ...
      <NdxEditorShell title="Kiosk Preferences">
        <div className="flex min-h-full min-w-0 flex-col gap-4 overflow-auto p-4">
          <p className="text-title font-semibold text-text-primary">Kiosk Mode</p>
          <p className="text-meta text-text-secondary">
            Restricts navigation to a chosen set of routes. Exiting kiosk mode always requires your
            real Lock PIN — there is no separate kiosk PIN.
          </p>
      ...
      <NdxToolWindow
        title="Kiosk Policy"
        subtitle={pinConfigured ? 'PIN ready' : 'PIN required'}
        side="right"
      >
        <div className="space-y-3 text-meta text-text-secondary">
          <p>Enable remains disabled until a real Lock PIN exists.</p>
          <p>Settings can be restricted by the same route allowlist gate.</p>
        </div>
      </NdxToolWindow>
    </div>
  )
