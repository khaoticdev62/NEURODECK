          ...
          <section className="border border-border bg-surface p-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-meta text-text-secondary">
                  Current data comes from persisted storage records and the system metrics storage
                  root. No real removable-media watcher or mount-point classifier exists yet.
                </p>
              </div>
          ...
          <section className="grid gap-3 lg:grid-cols-2">
            {storageDevices.length === 0 ? (
              <div className="border border-border bg-surface p-3">
                ...
                <p className="text-meta text-text-secondary">
                  NeuroDeck has no persisted storage records and no removable-storage backend yet.
                </p>
              </div>
            ) : (
              storageDevices.map((device) => <StorageDeviceCard key={device.id} device={device} />)
            )}
          </section>
