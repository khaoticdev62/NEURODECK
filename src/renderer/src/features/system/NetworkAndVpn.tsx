    ...
    setPrimary(
      ...
      <NdxToolWindow title="Network Tools" subtitle="Diagnostics">
        ...
        <div className="border-t border-border pt-3">
          <p className="text-meta font-semibold text-text-primary">Available data</p>
          <p className="text-meta text-text-tertiary">
            Interfaces, connections, DNS, proxy, VPN, and firewall are reported only when their
            collectors return real data.
          </p>
        </div>
      </NdxToolWindow>
    )
          ...
          {diagnostics && (
            <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                ...
                <ul className="space-y-1">
                  {diagnostics.interfaces.value?.map((iface) => (
                    <li key={iface.name} className="text-meta text-text-secondary">
                      {iface.name}: {iface.addressCount} address
                      {iface.addressCount === 1 ? '' : 'es'}
              ...
              <section className="space-y-2 ndx-settings-section">
                ...
                <DisabledAction
                  label="Ethernet"
                  reason="No Ethernet adapter integration exists yet."
                />
                <DisabledAction
                  label="VPN profiles"
