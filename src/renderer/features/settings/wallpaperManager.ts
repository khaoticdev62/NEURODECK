...
export class WallpaperManager {
  ...
  stop() {
    ...
    if (css) {
      css.style.opacity = "0";
      css.className = "app-background-css";
    }
  ...
  start(rawType: string) {
    ...
    if (type.startsWith("css-")) {
      if (css) {
        css.className = `app-background-css ${type}`;
  ...
  private randomLog() {
    return LOG_LINES[Math.floor(Math.random() * LOG_LINES.length)];
  }
  ...
  private drawStarfield(w: number, h: number, ac: string, rc: string) {
    ...
    const cx = w / 2,
      cy = h / 2,
      speed = 4;
    for (const star of this.particles as Array<{
      x: number;
      y: number;
      z: number;
      color: string;
  ...
  private drawParticles(w: number, h: number, ac: string) {
    ...
    for (const p of this.particles as Array<{
      x: number;
      y: number;
      vx: number;
      vy: number;
    ...
    for (let i = 0; i < ps.length; i++)
      for (let j = i + 1; j < ps.length; j++) {
        ...
        if (distSq < 10000) {
          // 100² — avoid sqrt for distant pairs
  ...
  private drawGrid(w: number, h: number, ac: string, rc: string) {
    ...
    for (let i = 0; i < 12; i++) {
      const ratio = (i + this.angle / 40) / 12,
        y = horizon + Math.pow(ratio, 2.5) * gh;
      ctx.strokeStyle = ac;
  ...
  private drawRadar(w: number, h: number, ac: string, rc: string) {
    ...
    ctx.lineTo(cx, cy + mr);
    ctx.stroke();
    for (const n of this.particles as Array<{
      x: number;
      y: number;
      size: number;
      alpha: number;
  ...
  private drawCircuit(w: number, h: number, ac: string, rc: string) {
    ...
    for (let i = 0; i < this.particles.length; i++) {
      const line = this.particles[i] as {
        points: Array<{ x: number; y: number }>;
        dirX: number;
        dirY: number;
        growSpeed: number;
      ...
      if (line.points.length > 0 && line.stepsRemaining > 0) {
        ...
        if (line.stepsRemaining <= 0 && Math.random() > 0.3 && line.points.length < 80) {
          ...
          line.dirX = Math.cos(a);
          line.dirY = Math.sin(a);
        }
      }
      if (line.points.length > 1) {
        ctx.strokeStyle = resolve(line.color);
        ...
        ctx.fillStyle = resolve(line.color);
      ...
      if (line.alpha <= 0 || (line.points.length >= 80 && line.stepsRemaining <= 0))
        this.particles[i] = this.makeCircuitLine(w, h);
    }
    ctx.globalAlpha = 1;
  }
  ...
  private drawAscii(w: number, h: number, ac: string) {
    ...
    ctx.fillStyle = ac;
    for (const line of this.particles as Array<{
      text: string;
      y: number;
      speed: number;
      alpha: number;
