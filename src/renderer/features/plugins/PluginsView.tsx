...
export function PluginsView({
  ...
  const [runtimeFilter, setRuntimeFilter] = useState<
    "all" | "lua" | "javascript" | "typescript" | "external"
  >("all");
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-nd-text-muted">
            Extension manager — loaded at sidecar startup
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 mb-4">
        <div className="rounded-xl border border-nd-border-subtle bg-nd-surface-secondary/40 px-4 py-3">
          <p className="text-[10px] uppercase font-bold tracking-wider text-nd-text-muted">
            Total Installed
          </p>
        ...
        <div className="rounded-xl border border-nd-accent-success/20 bg-nd-accent-success/[0.04] px-4 py-3">
          <p className="text-[10px] uppercase font-bold tracking-wider text-nd-accent-success/80">
            Active Scripts
          </p>
        ...
        <div
          className={`rounded-xl border px-4 py-3 transition ${totalErrors > 0 ? "border-nd-accent-error/30 bg-nd-accent-error/[0.04]" : "border-nd-border-subtle bg-nd-surface-secondary/40"}`}
        >
          <p
            className={`text-[10px] uppercase font-bold tracking-wider ${totalErrors > 0 ? "text-nd-accent-error/80" : "text-nd-text-muted"}`}
          >
            QA Failures
          </p>
          <p
            className={`text-xl font-bold mt-0.5 ${totalErrors > 0 ? "text-nd-accent-error" : "text-nd-text-primary"}`}
          >
            {totalErrors}
          </p>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2.5 scrollbar-thin">
            {loading && plugins.length === 0 ? (
              ...
              <EmptyState
                ...
                description={
                  plugins.length === 0
                    ? "Place Lua scripts in the plugins folder or install via URL."
              ...
              filteredPlugins.map((p) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <FileCode
                            className={`h-4 w-4 shrink-0 ${p.enabled ? "text-nd-accent-primary" : "text-nd-text-muted"}`}
                            aria-hidden="true"
                          />
                          <h4 className="font-semibold text-xs text-nd-text-primary truncate">
                            {p.name}
                          </h4>
                          <Badge
                        ...
                        <p className="text-[10px] text-nd-text-muted mt-0.5 font-mono truncate">
                          {p.file_name}
                        </p>
                      </div>
                    ...
                    <p className="text-xs text-nd-text-muted/80 line-clamp-1">
          ...
          {!selectedPlugin ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center text-nd-text-muted select-none">
              ...
              <p className="text-[10px] mt-1 max-w-[200px]">
                Select any active or installed plugin to inspect its manifest, permissions, and
                security audit log.
              </p>
            </div>
          ) : (
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="px-5 py-4 border-b border-nd-border-subtle bg-nd-surface-secondary/40">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <h3 className="text-sm font-semibold text-nd-text-primary truncate">
                      {selectedPlugin.name}
                    </h3>
                    <p className="text-[10px] text-nd-text-muted font-mono truncate mt-0.5">
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-5 scrollbar-thin">
                ...
                <div className="space-y-1.5">
                  <h5 className="text-[10px] uppercase font-bold tracking-wider text-nd-text-muted">
                    Description
                  </h5>
                  <p className="text-xs text-nd-text-secondary/90 leading-normal">
                ...
                <div className="space-y-1.5 border-t border-nd-text-muted/10 pt-4">
                  <h5 className="text-[10px] uppercase font-bold tracking-wider text-nd-text-muted mb-2">
                    Manifest Details
                  </h5>
                  <div className="grid grid-cols-2 gap-y-2 text-[11px]">
                    <div>
                      <span className="text-nd-text-muted block">Author</span>
                      <span className="text-nd-text-primary font-semibold">
                    ...
                    <div>
                      ...
                      <span className="text-nd-text-primary font-semibold">
                        v{selectedPlugin.version || "1.0.0"}
                      </span>
                    </div>
                    <div>
                      <span className="text-nd-text-muted block">Runtime</span>
                      <span className="text-nd-text-primary font-semibold capitalize">
                        {getPluginRuntime(selectedPlugin.file_name)}
                      </span>
                    </div>
                    <div>
                      <span className="text-nd-text-muted block">Marketplace</span>
                      <span className="text-nd-text-primary font-semibold">
                        {selectedPlugin.marketplace ? "Yes" : "Local"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 border-t border-nd-text-muted/10 pt-4">
                  ...
                  {validating ? (
                        ...
                        return (
                          <div className="p-3 rounded-xl border border-nd-accent-success/20 bg-nd-accent-success/[0.03] space-y-1">
                            ...
                            <p className="text-[10px] text-nd-text-muted">
                              No security or compiler syntax issues discovered inside the script.
                            </p>
                          </div>
                        );
                      } else {
                        return (
                          <div role="alert" className="p-3 rounded-xl border border-nd-accent-error/20 bg-nd-accent-error/[0.03] space-y-2">
                            <div className="flex items-center gap-1.5 text-xs font-semibold text-nd-accent-error">
                              <AlertTriangle className="h-4 w-4" aria-hidden="true" /> Validation Warnings/Errors
                            </div>
                            <ul className="list-disc pl-4 space-y-1 text-[10px] text-nd-text-muted">
                              {report.errors.map((err, i) => (
                                <li key={i} className="text-nd-accent-error/90">
                                  {err}
                                </li>
                              ))}
                              {report.warnings.map((warn, i) => (
                                <li key={i} className="text-nd-accent-warning">
                                  {warn}
                                </li>
                              ))}
                            </ul>
                    ...
                    <div className="p-3 rounded-xl border border-nd-text-muted/10 bg-nd-surface/30 text-[10px] text-nd-text-muted">
                      Validation not yet performed. Click 'Run Audit' above to scan the plugin's
                      code integrity.
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2 border-t border-nd-text-muted/10 pt-4">
                  ...
                  {selectedPlugin.permissions.length === 0 ? (
                    <p className="text-xs text-nd-text-muted">
                      No security permissions requested by this plugin.
                    </p>
                  ) : (
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="font-semibold text-nd-text-primary font-mono truncate">
                                {perm}
                              </p>
                              <p className="text-[10px] text-nd-text-muted mt-0.5">
                                {perm === "shell_execution" &&
                                  "Executes system shell binary commands and utilities."}
                                {perm === "filesystem_write" &&
                                  "Creates, deletes, or writes content to local codebase files."}
                                {perm === "filesystem_read" &&
                                  "Reads content from local codebase directory paths."}
                                {perm === "network_access" &&
                                  "Makes outgoing HTTP requests to remote network addresses."}
                                {perm === "clipboard_access" &&
                                  "Reads or writes to the operating system copy/paste buffer."}
                                {perm === "model_access" &&
                                  "Queries configured LLMs for completions."}
                                {perm === "session_access" &&
                                  "Inspects chat logging and session state databases."}
                                {perm === "memory_access" &&
                                  "Queries and pins SQLite vector database facts."}
                                {perm === "notification_access" &&
                                  "Displays desktop notifications and warning alerts."}
                              </p>
                            </div>
                          </div>
                        );
              ...
              <div className="px-5 py-4 border-t border-nd-text-muted/10 bg-nd-surface/30 flex items-center justify-between gap-2.5">
                <Button variant="secondary" fullWidth onClick={copyDiagnostics} icon={Copy}>
      ...
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-sm text-nd-text-primary">
                Allow high-risk access permissions?
              </p>
              <p className="mt-1 text-xs text-nd-text-muted">
                The plugin{" "}
                <strong className="text-nd-text-primary font-semibold">
                  "{showPermissionWarning?.name}"
                </strong>{" "}
                requests elevated security access. Enabling it will grant these permissions
                immediately.
              </p>
            </div>
          </div>

          <div role="alert" className="rounded-xl border border-nd-accent-error/20 bg-nd-accent-error/[0.03] p-4 space-y-3">
            <h6 className="text-[10px] uppercase font-bold tracking-wider text-nd-accent-error">
              Requested High-Risk Items:
            </h6>
            <div className="space-y-2">
              {showPermissionWarning?.permissions
                .filter((perm) => getPermissionRisk(perm) === "high")
                .map((perm) => (
                  <div key={perm} className="text-xs">
                    <p className="font-semibold text-nd-text-primary font-mono">{perm}</p>
                    <p className="text-[10px] text-nd-text-muted mt-0.5">
                      {perm === "shell_execution" &&
                        "Allows the script to invoke host operating system binary CLI commands and scripts."}
                      {perm === "filesystem_write" &&
                        "Allows the script to create, modify, or delete local files."}
                    </p>
                  </div>
                ))}
            </div>
          ...
          <p className="text-[11px] text-nd-text-muted leading-relaxed">
            Only enable plugins from authors you completely trust. Malicious scripts can read,
            write, or execute arbitrary operations on your local machine.
          </p>
        </div>
      </Modal>
