    ...
    ? [
        {
          ...
          value: diag.mdns_active ? "Active" : "Inactive",
          ok: diag.mdns_active,
        },
        ...
        {
          label: "Group code set",
          value: diag.group_code_set ? "Yes" : "No (using DEFAULT)",
          ok: diag.group_code_set,
        },
          ...
          <>
            <div className="overflow-hidden rounded-xl border border-nd-border-subtle bg-nd-surface-secondary/40">
              <table
                ...
                role="table"
                aria-label="Transfer diagnostic results"
              >
                <thead>
                  <tr className="border-b border-nd-border-subtle bg-nd-surface-secondary/60">
                    <th
                      scope="col"
                      className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-[0.14em] text-nd-text-muted"
                    >
                      Check
                    </th>
                    <th
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr
                      key={row.label}
                      className="border-b border-nd-border-subtle/50 last:border-0"
                    >
                      <td className="px-4 py-2.5 text-nd-text-secondary">{row.label}</td>
                      <td className="px-4 py-2.5 font-mono text-xs text-nd-text-primary break-all">
                        {row.value}
                      </td>
            ...
            <div className="rounded-xl border border-nd-border-subtle bg-nd-surface-secondary/40 p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1 text-xs text-nd-text-muted">
                  ...
                  <p>
                    TCP port <span className="font-mono text-nd-accent-primary">18338</span> —
                    NEURODECK P2P transfer
                  </p>
                  <p>
                    TCP port <span className="font-mono text-nd-accent-primary">42000</span> —
                    Warpinator gRPC
                  </p>
                  <p>
                    UDP port <span className="font-mono text-nd-accent-primary">5353</span> — mDNS
                    peer discovery (multicast)
                  </p>
                  <p className="pt-1">
                    If peers are not discovered, ensure mDNS (multicast) is allowed on your
                    router/firewall. For VPN peers, add them manually in the Devices tab.
                  </p>
                </div>
              </div>
            </div>
