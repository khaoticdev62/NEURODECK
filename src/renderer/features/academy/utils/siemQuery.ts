...
export function parseQuery(raw: string): ParsedQuery | QueryError {
  ...
  while (i < tokens.length) {
    ...
    if (term.includes(":")) {
      ...
      if (!field || !value)
        return { message: `Invalid field term: "${term}". Use field:value syntax.` };
...
export function runQuery(events: SiemEvent[], query: ParsedQuery): SiemEvent[] {
  ...
  return events.filter((event) => {
    ...
    return query.operator === "OR" ? results.some(Boolean) : results.every(Boolean);
  });
}
...
export function executeQuery(
  events: SiemEvent[],
  raw: string
): { results: SiemEvent[]; error?: string; query?: ParsedQuery } {
...
export const SIEM_CHALLENGES: SiemChallenge[] = [
  {
    ...
    description:
      "Find all authentication failure events that originated from external (non-RFC1918) IP addresses. These are potential brute-force or credential stuffing attempts.",
  ...
  },
  {
    ...
    description:
      "Identify all PowerShell process execution events across all hosts. Look for encoded commands, bypass flags, and suspicious parents.",
  ...
  },
  {
    ...
    description:
      "Find all events involving the Finance department subnet (10.20.x.x). Review for unusual access patterns.",
  ...
  },
  {
    ...
    description:
      "Identify high-severity firewall or proxy events that indicate large data transfers leaving the network. These are potential exfiltration indicators.",
  ...
  },
  {
    ...
    description:
      "Find all DNS queries and network events to known C2 infrastructure (91.234.55.119 or cdn-static.company-update.net).",
...
export function gradeChallenge(
  ...
  const fractionFound =
    challenge.requiredEventIds.length > 0
