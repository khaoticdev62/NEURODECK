...
describe("reducer", () => {
  ...
  it("updates controller settings and deck mode independently", () => {
    const next = dispatch({
      ...
      settings: { enabled: false } as Partial<typeof controllerDefaults>,
    });
    expect(next.controllerSettings.enabled).toBe(false);
    expect(next.controllerSettings.preferredProfile).toBe(controllerDefaults.preferredProfile);
  ...
  it("updates agent, model, plugin, and memory slices", () => {
    ...
    const plugin = neurodeckReducer(model, {
      type: "toggle-plugin",
      id: "plugin-1",
    });
