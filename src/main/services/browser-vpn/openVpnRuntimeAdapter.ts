...
export class OpenVpnRuntimeAdapter {
  ...
  prepareConfig(profileId: string, configText: string): string {
    ...
    const filePath = path.join(dir, `${profileId}.ovpn`);
  ...
  connect(profileId: string, configPath: string): { ok: boolean; error?: string; pid?: number } {
    try {
      const child = spawn("openvpn", ["--config", configPath], {
...
export const openVpnRuntimeAdapter = new OpenVpnRuntimeAdapter();
