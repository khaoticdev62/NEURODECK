...
_REDACTION_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    # Generic API keys
    (re.compile(r"(?i)(api[_-]?key\s*[:=]\s*)['\"]?[a-z0-9_\-]{16,}['\"]?"), r"\1[REDACTED_API_KEY]"),
    # Generic tokens
    ...
    # Database URLs
    (re.compile(r"(?i)(DATABASE_URL\s*[:=]\s*)['\"]?[^\s'\"]+['\"]?"), r"\1[REDACTED_DATABASE_URL]"),
    ...
    (re.compile(r"(?i)(client_secret\s*[:=]\s*)['\"]?[^\s'\"]+['\"]?"), r"\1[REDACTED_CLIENT_SECRET]"),
    # GCP service account keys
    (re.compile(r"\"type\":\s*\"service_account\".*?\"private_key\":\s*\"-----BEGIN PRIVATE KEY-----[\\s\\S]*?-----END PRIVATE KEY-----\"", re.DOTALL), "[REDACTED_GCP_SERVICE_ACCOUNT]"),
    # Azure keys
    (re.compile(r"[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}"), "[REDACTED_UUID]"),
    # SSH keys inline
