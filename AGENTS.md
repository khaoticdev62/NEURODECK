...
- **Tailwind surface namespace is canonical.** `frontend/tailwind.config.js` exposes the DS surface namespace as `nd.surface-primary/secondary/tertiary` mapped to `--nd-surface-primary/secondary/tertiary`. The runtime injector writes those same canonical variables, so static CSS and injected themes agree. `nd.surface-base/raised` are deprecated aliases that resolve to primary/secondary; the duplicate `surface.*`, `text-ds.*`, `accent.*`, and `border.*` alias layers have been removed. Prefer `bg-nd-surface-primary`, `text-nd-text-primary`, `border-nd-border-subtle`, `shadow-card`, etc.
