...
**Repository reconciliation:** 2026-06-23 with Epic 11 complete. Current validation baseline: 497 unit/integration tests across 102 files, full TypeScript checks, lint, production build, and the Electron boot smoke test pass.
...
### Epic 3 — Onboarding and global UX ⚠️ partially complete (8 of 12 screens real; 4 deferred — see ledger)
...
### Epic 5 — Workspaces and files ⚠️ read/write/delete core real; copy/move/rename/compress/extract remain
...
### Epic 6 — Terminal and Git ⚠️ real local/remote Git core including restore/branch-delete/force-push; advanced terminal modes remain

- [ ] Terminal Service / PTY (§21) — **partially real, search/copy selection now real**: `node-pty` local shells, multiple workspace-scoped sessions, resize, streaming output, bounded snapshots, cancellation, exit status, working directory, sanitized inherited environment, typed IPC, real scrollback search (`@xterm/addon-search`), and real selection-to-clipboard copy are real. SSH (real, but via Remote Systems' own session type — Universal Terminal itself stays local-PTY-only), headless execution, and AI intent proposals remain. "History" beyond the shell's own up/down-arrow command history and xterm's 5000-line scrollback buffer is not a separate feature this slice builds
- [ ] ND-028 Universal Terminal — **partially real, search/copy real**: `/terminal` Direct mode uses xterm.js over real PTY sessions with controller-focusable create/select/focus/terminate controls, resize, ordered streaming, exit state, workspace/branch context, reviewed termination, a real Find bar (Previous/Next over the real scrollback buffer), and a real Copy selection action. Command Builder is real at `/terminal/builder`; integrated Intent/Split/Remote modes and AI suggestions remain
...
### Epic 11 — System integration ⚠️ System Metrics, Recovery, System Dashboard, Controller Settings, Display/Theme Settings, Network/VPN (read-only), Privacy and Permissions, Integrations (read-only), Updates (feed check), Quick Access Overlay, Power Menu (safe subset), and About/Diagnostics real; remaining management integrations remain
