...
### Epic 8 — Agents and workflows ⚠️ Workflow Engine real; Agent Runtime core real, IPC/UI/tool execution remain
...
- [ ] ND-016 Agent Operations Center — **not built**: persisted Agent Runtime records now exist; typed IPC, real activity streaming, controller UI, pause/restrict/archive/report actions remain
- [ ] ND-017 Agent Detail — **not built**: persisted definitions, timelines, output, model identity, and token usage exist; typed IPC and the Overview/Timeline/Files/Tools/Permissions/Logs/Output UI remain
