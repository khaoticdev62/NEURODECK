...
### Epic 8 — Agents and workflows ⚠️ Workflow Engine real; Agent Runtime core + IPC + UI real, tool execution remains

- [ ] Agent Runtime (§17) — **core lifecycle, typed IPC, and UI real; tool execution remains**: persisted definitions and runs include every required identity/scope/policy/resource field; planning uses the real Epic 9 router; cancellation aborts provider requests; all state transitions and token usage are persisted. `registerAgentHandlers.ts` exposes real CRUD/run-control IPC, with live run updates pushed to the renderer (`agentRun.update`, the same pattern terminal data/exit events use). ActionQueue-backed tool execution, pause/resume, and child-agent bounds remain for subsequent sprint items — an agent run today can only produce a text plan, never a submitted tool call.
...
### Epic 11 — System integration ⚠️ System Metrics, Recovery, System Dashboard, Controller Settings, Display/Theme Settings, Power Menu (safe subset), and About/Diagnostics real; remaining settings integrations remain
